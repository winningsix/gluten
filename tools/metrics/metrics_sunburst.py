...
L1_GROUPS = [
    ("Native Execution", ["nativeHasNextNanos", "nativeNextNanos"], True),
    ("Shuffle Write (JNI)", ["shuffleWriteJniNanos"], False),
    ("Shuffle Write (stop)", ["shuffleWriteStopNanos"], False),
    (
        "Shuffle Write (init+meta)",
        ["shuffleWriterInitNanos", "shuffleWriteMetaNanos"],
        False,
    ),
    ("Shuffle Read Init", ["shuffleReadInitNanos"], False),
    ("Plan Build", ["planBuildNanos"], False),
    ("Arrow Import", ["arrowImportNanos"], False),
    ("Broadcast Build", ["broadcastBuildNanos"], False),
    ("Task Cleanup", ["taskCleanupNanos"], False),
]

OPERATOR_PRIMARY_METRIC = {
    "FileSourceScanExecTransformer parquet": "time of scan and filter",
    "CudfFilterExecTransformer": "time of filter",
    "CudfProjectExecTransformer": "time of project",
    "CudfFlushableHashAggregateExecTransformer": "time of aggregation",
    "CudfRegularHashAggregateExecTransformer": "time of aggregation",
    "CudfSortExecTransformer": "time of sort",
    "CudfWriteFilesExecTransformer": "time of write",
    "CudfInputIteratorTransformer": None,
    "CudfShuffledHashJoinExecTransformer": None,
    "CudfBroadcastHashJoinExecTransformer": None,
    "TakeOrderedAndProjectExecTransformer": "time of sort",
    "CudfWindowExecTransformer": "time of window",
    "CudfExpandExecTransformer": "time of expand",
    "CudfNestedLoopJoinExecTransformer": None,
    "CudfTopNRowNumberExecTransformer": "time of sort",
}
...
DISPLAY_NAMES = {
    ...
    "CudfFlushableHashAggregateExecTransformer": "HashAgg (partial)",
    "CudfRegularHashAggregateExecTransformer": "HashAgg (final)",
    ...
    with open(path) as f:
        for line in f:
            ...
            for acc in info.get("Accumulables", []):
                if acc.get("Name") == "internal.metrics.executorRunTime":
...
def aggregate(spark_tasks, tracker_tasks):
    ...
    for tid in matched:
        ...
        native_exec = tracker["nativeHasNextNanos"] + tracker["nativeNextNanos"]
        if native_exec <= 0:
            continue
        n_tasks += 1
...
def build_acc_map(event_log_path):
    ...
    with open(event_log_path) as f:
        for line in f:
            ...
            ev = json.loads(line)
            if (
                ev.get("Event") != "org.apache.spark.sql.execution.ui"
...
def _walk_acc(node, acc_map):
    ...
    for m in node.get("metrics", []):
        acc_map[m["accumulatorId"]] = (m["name"], name, m["metricType"])
...
def aggregate_operators_with_gpu(event_log_path, tracker_tasks, spark_tasks):
    ...
    for aid, (mname, opname, mtype) in acc_map.items():
        ...
        if mname == GPU_COMPUTE_METRIC:
            gpu_ids.add(aid)
            display = DISPLAY_NAMES.get(matched_op, opname.strip())
        ...
        if primary is not None and mname == primary:
            ...
            matched_op == "CudfInputIteratorTransformer" and mname in INPUT_ITER_METRICS
        ):
            wall_ids.add(aid)
        elif (
            matched_op
    ...
    task_accs = parse_task_accumulables(event_log_path, all_ids)

    matched = (
        set(spark_tasks.keys()) & set(tracker_tasks.keys()) & set(task_accs.keys())
    )
    ...
    for tid in matched:
        tracker = tracker_tasks[tid]
        native = tracker["nativeHasNextNanos"] + tracker["nativeNextNanos"]
        if native <= 0:
            continue
        n += 1
        total_native += native
...
def build_l2_slices(op_totals, total_native, min_pct):
    slices = []
    accounted = 0
    for label, val in sorted(op_totals.items(), key=lambda x: -x[1]):
        if val > 0:
            slices.append((label, val))
            accounted += val
...
def draw_sunburst_3ring(
    ...
    w1, _ = ax.pie(
        ...
        startangle=90,
        counterclock=False,
        wedgeprops=dict(width=R1_OUTER - R1_INNER, edgecolor="white", linewidth=1.5),
    )
    ...
    w2, _ = ax.pie(
        ...
        startangle=90,
        counterclock=False,
        wedgeprops=dict(width=R2_OUTER - R2_INNER, edgecolor="white", linewidth=1.0),
    )
    ...
    for i, (l2_label, l2_val) in enumerate(zip(r2_labels, r2_vals)):
        if l3_map and l2_label in l3_map and l3_map[l2_label] is not None:
    ...
    w3, _ = ax.pie(
        ...
        startangle=90,
        counterclock=False,
        wedgeprops=dict(width=R3_OUTER - R3_INNER, edgecolor="white", linewidth=0.6),
    )

    # Annotate L3 wedges that are large enough
    for wedge, label, parent, val in zip(w3, r3_labels, r3_parent_labels, r3_vals):
        if not label:
            continue
        span = abs(wedge.theta2 - wedge.theta1)
        if span < 6:
            continue
        parent_wall = dict(zip(r2_labels, r2_vals)).get(parent, 1)
        pct = val / parent_wall * 100 if parent_wall > 0 else 0
        mid_rad = _wedge_mid_angle(wedge)
        x, y = _polar_to_xy(mid_rad, (R3_INNER + R3_OUTER) / 2)
        ...
        ax.text(
            ...
            color="white",
            bbox=dict(
                boxstyle="round,pad=0.1", facecolor="black", alpha=0.4, edgecolor="none"
            ),
        )

    # ---- Compact legend for ring meanings + L3 colors ----
    legend_items = [
        mpatches.Patch(facecolor="gray", alpha=0.5, edgecolor="white"),
        mpatches.Patch(facecolor=GPU_COLOR, edgecolor="white"),
        mpatches.Patch(facecolor=CPU_COLOR, edgecolor="white"),
    ]
    legend_labels = [
...
def main():
    parser = argparse.ArgumentParser(
        description="3-ring sunburst chart of task wall-time " "with GPU/CPU split"
    )
    parser.add_argument("event_log", help="Spark event log file")
    parser.add_argument(
        "executor_log", help="Executor stderr with " "[TASK_TIMING] lines"
    )
    parser.add_argument(
        "--output", "-o", default="metrics_sunburst.png", help="Output image path"
    )
    parser.add_argument(
        "--min-pct",
        type=float,
        default=1.0,
        help="Merge slices below this %% " "threshold",
    )
    parser.add_argument("--title-suffix", default="", help="Extra text for chart title")
    ...
    totals, total_wall, total_exec_run, n_tasks = aggregate(spark_tasks, tracker_tasks)
    ...
    # --- Level 1 ---
    l1_slices = build_l1_slices(totals, total_wall, args.min_pct)

    print(f"\n=== Level 1 (total wall = " f"{total_wall / 1e6:,.1f} ms) ===")
    for label, val in l1_slices:
        pct = val / total_wall * 100
        print(f"  {label:30s} {val / 1e6:>10.1f} ms  " f"({pct:5.1f}%)")

    # --- Level 2 + GPU compute ---
    op_wall, op_gpu, total_native, n_l2 = aggregate_operators_with_gpu(
        args.event_log, tracker_tasks, spark_tasks
    )

    print(f"\n=== Level 2: Native Execution breakdown " f"({n_l2} tasks) ===")
    print(f"Total nativeExec = {total_native / 1e6:,.1f} ms")
    for label, val in sorted(op_wall.items(), key=lambda x: -x[1]):
    ...
    l2_slices = build_l2_slices(op_wall, total_native, args.min_pct)
