...
/**
 * JNI bridge to the C++ FluxQueryCoordinator for multi-fragment plan execution with streaming
 * exchange.
 *
 * <p>This enables Velox-native multi-stage query execution (FLUX) within a single process. Multiple
 ...
 *
 * <ol>
 *   <li>{@link #nativeCreateFluxQuery} — parse Substrait plans, build fragments, create coordinator
 *   <li>{@link #nativeStartFluxQuery} — launch all fragments concurrently (all-stages-up)
 *   <li>{@link #nativeGetFluxOutput} — pull output batches from root fragment (blocking)
 *   <li>{@link #nativeAbortFluxQuery} — optionally request cooperative abort without releasing
 *       handle
 *   <li>{@link #nativeCloseFluxQuery} — abort remaining tasks and release resources
 * </ol>
 *
 ...
 */
public class FluxQueryJniWrapper implements RuntimeAware {
  private final Runtime runtime;

  private FluxQueryJniWrapper(Runtime runtime) {
    this.runtime = runtime;
  }

  public static FluxQueryJniWrapper create(Runtime runtime) {
    return new FluxQueryJniWrapper(runtime);
  }
  ...
  /**
   * Create an FLUX query coordinator with multiple plan fragments and exchanges.
   *
   * @param substraitPlans array of Substrait plan bytes, one per fragment (index = fragment id).
   *     Fragment 0 is the root whose output is returned by {@link #nativeGetFluxOutput}.
   ...
   * @param fluxPeerSpecJson UTF-8 JSON object with the native query id, local peer id, and optional
   ...
   */
  public native long nativeCreateFluxQuery(
      byte[][] substraitPlans,
      int[] numDriversPerFragment,
      byte[] exchangeSpecsJson,
      byte[] fluxPeerSpecJson,
  ...
  /**
   * Build the FLUX fragments and return the final Velox plan text for each fragment without
   * starting execution.
   *
   * <p>The returned plans are captured after the same FLUX rewrites used by {@link
   * #nativeCreateFluxQuery}: ValueStream inputs are replaced with Exchange nodes and every fragment
   * is wrapped with a PartitionedOutput node.
   */
  public native String[] nativeExplainFluxQuery(
      byte[][] substraitPlans,
      int[] numDriversPerFragment,
      byte[] exchangeSpecsJson,
      byte[][][] splitInfosPerFragment,
   ...
   * {@link #nativeCreateFluxQuery}.
   *
   * @param handle native handle returned by {@link #nativeCreateFluxQuery}.
   * @throws RuntimeException if already started or if a fragment fails to launch.
   */
  public native void nativeStartFluxQuery(long handle);
   ...
   */
  public native long nativeGetFluxOutput(long handle);

  /**
   * Request cooperative abort of every Velox task owned by this FLUX coordinator. This does not
   * release {@code handle}; the Spark task thread must still call {@link #nativeCloseFluxQuery}.
   *
   * @param handle native handle returned by {@link #nativeCreateFluxQuery}.
   */
  public native void nativeAbortFluxQuery(long handle);

  /**
   * Close the FLUX query: abort any running fragments and release native resources. Must be called
   * exactly once. The caller is responsible for guarding against double-close (same pattern as
   * {@link ColumnarBatchOutIterator}).
   *
   * @param handle native handle returned by {@link #nativeCreateFluxQuery}.
   */
  public native void nativeCloseFluxQuery(long handle);
}
