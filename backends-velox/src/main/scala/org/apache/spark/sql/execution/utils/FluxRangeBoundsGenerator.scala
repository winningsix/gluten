...
/**
 * Computes Spark-compatible range bounds for an FLUX exchange.
 *
 ...
 */
object FluxRangeBoundsGenerator extends Logging {
  ...
  /**
   * Bounds cache scoped to one FLUX query launch. The launch owner supplies its Spark SQL execution
    ...
    def getOrCompute(
      ...
      if (owner) {
        try {
          ...
          require(generated != null, "FLUX RANGE bounds generator returned null")
  ...
  def generate(
    ...
    require(samplePlan != null, "FLUX RANGE requires a producer plan for bounded sampling")
    require(
      requestedPartitions > 0,
      s"FLUX RANGE requires positive partitions: $requestedPartitions")
    require(ordering.nonEmpty, "FLUX RANGE requires at least one SortOrder")
    ordering.foreach {
      order =>
        require(
          order.child.isInstanceOf[Attribute],
          s"FLUX RANGE D1 only supports attribute sort keys, found ${order.child.sql}")
        require(
          supports(order.dataType),
          s"FLUX RANGE D1 does not support sort-key type ${order.dataType.catalogString}")
    }
    ...
    // submits all missing parents together. Multi-peer FLUX tasks cannot make progress until all
  ...
  private[utils] def materializeShuffleDependencies(rdd: RDD[_]): Unit = {
    ...
    if (dependencies.nonEmpty) {
      logInfo(
        s"FLUX RANGE preparation is materializing ${dependencies.size} shuffle map stage(s) " +
          s"serially to preserve multi-peer FLUX gang progress: " +
  ...
  private[utils] def determineBounds(
    ...
    require(
      ...
      s"FLUX RANGE sampling returned ${sketches.length} sketches for $inputPartitions inputs")
    ...
    require(
      ...
      s"FLUX RANGE retained $retainedRows sample rows, exceeding the $targetRows-row limit")
    require(
      retainedBytes <= maxSampleBytes,
      s"FLUX RANGE retained $retainedBytes serialized key bytes, exceeding the " +
    ...
    sketches.zipWithIndex.foreach {
      case (sketch, partitionId) if sketch.count > 0L && sketch.samples.isEmpty =>
        throw new IllegalArgumentException(
          s"FLUX RANGE input partition $partitionId had ${sketch.count} rows but produced no " +
            "bounded samples")
      case _ =>
    }
    ...
    require(
      ...
      s"FLUX RANGE found $numItems input rows but only $retainedRows bounded samples for " +
  ...
  private[utils] def sketchPartition(
    ...
    require(maxRows > 0, s"FLUX RANGE partition sample row limit must be positive: $maxRows")
    require(
      maxSerializedKeyBytes > 0L,
      s"FLUX RANGE partition sample byte limit must be positive: $maxSerializedKeyBytes")
      ...
      row =>
        ...
        if (retained.size < maxRows) {
          ...
          require(
            ...
            s"FLUX RANGE selected reservoir keys require $admittedBytes serialized bytes, " +
          ...
          require(
            ...
            s"FLUX RANGE selected reservoir keys require $replacementBytes serialized bytes, " +
              s"exceeding the $maxSerializedKeyBytes-byte partition budget; increase " +
              s"$MaxSampleBytesKey"
          )
          retained.dequeue()
  ...
  private[utils] def sketchColumnarPartition(
    ...
    require(
      maxSerializedKeyBytes > 0L,
      s"FLUX RANGE partition sample byte limit must be positive: $maxSerializedKeyBytes")
    require(keyOrdinals.nonEmpty, "FLUX RANGE sampling requires at least one key column")
        ...
        if (local.nonEmpty) {
          ...
          try {
            ...
            require(
              ...
              s"FLUX RANGE native sample returned ${selectedRows.length} rows for " +
    ...
    require(
      ...
      s"FLUX RANGE selected reservoir keys require $serializedKeyBytes serialized bytes, " +
        s"exceeding the $maxSerializedKeyBytes-byte partition budget; increase " +
        s"$MaxSampleBytesKey"
    )
    PartitionSketch(count, selected, serializedKeyBytes)
  ...
  private def orderingOrdinals(
    ...
    ordering.map {
      order =>
        ...
        require(
          ordinal >= 0,
          s"FLUX RANGE sort key ${attribute.sql} is absent from producer output")
        ordinal
    }.toArray
  }
  ...
  private def validateGlobalLimits(
    ...
    require(
      inputPartitions > 0,
      s"FLUX RANGE input partition count must be positive: $inputPartitions")
    require(
      requestedPartitions > 0,
      s"FLUX RANGE partition count must be positive: $requestedPartitions")
    require(
      samplePointsPerPartitionHint > 0,
      s"FLUX RANGE sample-points hint must be positive: $samplePointsPerPartitionHint")
    ...
    require(
      ...
      s"FLUX RANGE has $inputPartitions input partitions but $MaxSampleRowsKey=$maxSampleRows; " +
        "at least one bounded sample slot per input partition is required"
    )
    require(
      maxSampleBytes / inputPartitions > 0L,
      s"FLUX RANGE has $inputPartitions input partitions but $MaxSampleBytesKey=$maxSampleBytes; " +
  ...
  private def cacheOrderingKey(
    ...
    require(ordering.nonEmpty, "FLUX RANGE cache requires at least one SortOrder")
    ordering.map {
      order =>
        val attribute = order.child match {
          ...
          case other =>
            throw new IllegalArgumentException(
              s"FLUX RANGE cache only supports attribute sort keys, found ${other.sql}")
        }
          ...
          index =>
            ...
            if (row.isNullAt(index)) {
              ...
              ordering(index).dataType match {
                ...
                case other =>
                  throw new IllegalArgumentException(
                    s"FLUX RANGE D1 cannot encode ${other.catalogString}")
              }
            }
        }
    }
