 ...
 * registry; C5 will let `MppNativeQueryExec` consult it instead of probing.
 *
 ...
 */
class GlutenMppEndpointRegistry {

  // executorId -> latest record. TrieMap is lock-free for reads and atomically updated.
  private val records = TrieMap.empty[String, MppExecutorEndpointRecord]
   ...
   */
  def register(record: MppExecutorEndpointRecord): Long = {
  ...
  def snapshot(): Seq[MppExecutorEndpointRecord] =
...
object GlutenMppEndpointRegistry {
  def apply(): GlutenMppEndpointRegistry = new GlutenMppEndpointRegistry()
}
