...
/**
 * Serializable protocol exchanged by [[GlutenMppExecutorService]] on each executor and
...
/**
 * Identifies one Spark stage attempt executing a native MPP query.
 *
 ...
 *
 * @param queryId
 *   Logical MPP query ID shared by all peers.
 * @param stageId
 *   Spark stage ID that owns the peer tasks.
 * @param stageAttemptNumber
 *   Spark attempt number for that stage.
 */
final case class MppQueryRunId(queryId: String, stageId: Int, stageAttemptNumber: Int)
  extends Serializable {
  def logId: String = s"$queryId/$stageId/$stageAttemptNumber"
}

/**
 * Executor-reported lifecycle of one native MPP peer.
 *
 * This is distinct from `MppQueryRunState`, which is the driver-owned aggregate state for all peers
 ...
 */
final case class MppPeerSnapshot(
    runId: MppQueryRunId,
    peerIndex: Int,
 ...
 */
final case class MppQueryFailure(
    eventId: String,
    runId: MppQueryRunId,
    peerIndex: Int,
    expectedPeerCount: Int,
 ...
 */
final case class MppTerminalEvent(
    eventId: String,
    runId: MppQueryRunId,
    peerIndex: Int,
    expectedPeerCount: Int,
...
final case class MppQueryHeartbeat(
    ...
    executorSessionId: String,
    sentAtMs: Long,
    peers: Seq[MppPeerSnapshot],
    failures: Seq[MppQueryFailure],
    terminals: Seq[MppTerminalEvent])
  extends GlutenMppControlMessage

/**
 * Driver command to abort one exact native MPP peer.
 *
 ...
 */
final case class MppAbortQuery(
    runId: MppQueryRunId,
    peerIndex: Int,
    taskAttemptId: Long,
    executorId: String,
