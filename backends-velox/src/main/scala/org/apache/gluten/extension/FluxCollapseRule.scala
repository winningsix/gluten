...
/**
 * Data class representing a native execution fragment in the FLUX plan.
 *
...
/**
 * FLUX plan collapse rule that replaces [[ColumnarCollapseTransformStages]] for queries that can be
 * fully executed in FLUX mode.
 *
 * In BSP (Bulk Synchronous Parallel) mode, Gluten breaks the plan at every ShuffleExchange into
 * separate stages, each wrapped in a [[WholeStageTransformer]]. In FLUX mode, ALL stages run
 ...
 *      [[FluxNativeQueryExec]]. 4. If any operator is NOT supported, returns the plan unchanged so
 *      that BSP mode (via [[ColumnarCollapseTransformStages]]) handles it.
 *
 * Gated by config: `spark.gluten.mpp.enabled` (default: false).
 */
case class FluxCollapseRule(glutenConf: GlutenConfig) extends Rule[SparkPlan] with Logging {
  ...
  override def apply(plan: SparkPlan): SparkPlan = {
    ...
    // When Plan C (FluxStrategy) is enabled, disable Plan D entirely.
    // FluxStrategy handles FLUX at the Strategy level; FluxCollapseRule
    ...
    if (
      ...
      // Plan C (FluxStrategy) handles non-write queries at the strategy level. But it
    ...
    // Plan C check: if FluxStrategy already claimed this plan
    if (alreadyHandledByFluxStrategy(plan)) {
      return plan
    }
    logWarning("FluxCollapseRule: attempting FLUX collapse on query plan")
    ...
    // into the main plan tree (so FLUX absorbs it as an additional fragment) instead of leaving it
    ...
    val preRewritten =
      if (isScalarSubqueryRewriteEnabled) {
        ...
        logWarning(
          s"FluxCollapseRule: keeping ScalarSubquery expressions materialized by Spark " +
            s"because $scalarSubqueryRewriteDisabledReason")
        plan
      }
    preRewritten match {
      case v2: V2TableWriteExec if !alreadyHandledByFluxStrategy(v2.query) =>
        ...
        // all scans, relational operators, and exchanges still execute in one FLUX
        // query and feed the V2 writer through the normal columnar transition.
        val writeNormalization = FluxCollapseRule.normalizeV2WriteExchangeAdapters(v2.query)
        if (writeNormalization.strippedAdapters > 0) {
          logInfo(
            s"FluxCollapseRule: removed ${writeNormalization.strippedAdapters} " +
              "V2-write-only ColumnarToRow convention adapter(s); " +
              "the V2 writer remains the row/commit boundary and FluxNativeQueryExec.doExecute " +
              "performs its final native-to-row conversion")
        }
        collapseFluxOrArrowHybrid(writeNormalization.plan) match {
          case Some(fluxQuery) =>
            logWarning(
              s"FluxCollapseRule: *** FLUX MODE ACTIVE UNDER V2 WRITE *** " +
                s"writer=${v2.getClass.getSimpleName}")
            val distributedWriteQuery = fluxQuery.transformDown {
              case flux: FluxNativeQueryExec => flux.copy(distributedWriteOutput = true)
            }
            v2.withNewChildren(Seq(distributedWriteQuery))
          case None =>
            val reason =
              s"FluxCollapseRule: FALLBACK TO BSP under V2 write " +
      ...
      case dwce: DataWritingCommandExec =>
        // Collapse the whole WriteFilesExecTransformer subtree (write + query) INTO FLUX: the
        ...
        // emits the WriteRel automatically. Keeping the write inside the pinned FLUX task avoids
        // the
        ...
        val rewrittenWrite = dwce.transformDown {
          // Guard with !collapsedWrite: tryCollapseFlux(wft) returns FluxNativeQueryExec(wft), and
        ...
        if (collapsedWrite) {
          ...
          val reason = "FluxCollapseRule: FALLBACK TO BSP (inside DataWritingCommandExec)"
          logWarning(reason)
          if (failOnFallback) throw new IllegalStateException(reason)
          plan
        }
      case _: ExecutedCommandExec =>
        ...
        // query child.  Strict FLUX fallback applies to data execution, not to this
        // zero-row command boundary.  The first action against the registered view
        // is planned independently and must still collapse to FLUX.
        preRewritten
      case _: CommandResultExec =>
        ...
        // inspection cannot discover the FLUX query that already ran. No data
      ...
      case _: V2CommandExec =>
        ...
        preRewritten
      case _ =>
        collapseFluxOrArrowHybrid(preRewritten).getOrElse {
          val reason = "FluxCollapseRule: FALLBACK TO BSP"
          logWarning(reason)
          if (failOnFallback) throw new IllegalStateException(reason)
          plan
        }
  ...
  /** Check if the plan tree already contains FluxNativeQueryExec (set by FluxStrategy / Plan C). */
  private def alreadyHandledByFluxStrategy(plan: SparkPlan): Boolean = {
    plan match {
      case _: FluxNativeQueryExec => true
      case _ => plan.children.exists(alreadyHandledByFluxStrategy)
    }
  }

  private def isFluxEnabled: Boolean = {
    SQLConf.get.getConfString(FLUX_ENABLED_KEY, FLUX_ENABLED_DEFAULT).toBoolean
  }
   ...
   * boundary. The latter is Phase B1: the native producer runs as FLUX, the unchanged Python
   ...
   */
  private def collapseFluxOrArrowHybrid(plan: SparkPlan): Option[SparkPlan] = {
    plan match {
      // Dataset.rdd is represented by a terminal DeserializeToObjectExec. Keep exactly that
      // object-producing root and its direct C2R outside FLUX while requiring the relational
      ...
      // so retain that semantically required driver boundary while collapsing its child to FLUX.
      // This avoids a BSP stage and never introduces the expensive C2R -> R2C round trip.
      case c2r: ColumnarToRowExecBase if FluxCollapseRule.isBoundedScalarResult(c2r.child) =>
        collapseFluxOrArrowHybrid(c2r.child).map {
          nativeChild =>
            logWarning(
              "FluxCollapseRule: *** FLUX MODE ACTIVE UNDER BOUNDED SCALAR ROW OUTPUT *** " +
                s"boundary=${c2r.getClass.getSimpleName}; final C2R converts at most one row")
            c2r.withNewChildren(Seq(nativeChild))
        }
      case c2r: ColumnarToRowExec if FluxCollapseRule.isBoundedScalarResult(c2r.child) =>
        collapseFluxOrArrowHybrid(c2r.child).map {
          nativeChild =>
            logWarning(
              "FluxCollapseRule: *** FLUX MODE ACTIVE UNDER BOUNDED SCALAR ROW OUTPUT *** " +
                s"boundary=${c2r.getClass.getSimpleName}; Spark C2R removed and " +
                "FluxNativeQueryExec performs the final JNI row conversion")
            nativeChild
        }
      ...
      // adapter outside FluxNativeQueryExec while requiring its entire child to be either native or
  ...
  private def collapseTerminalRowEgress(
    ...
    tryCollapseJvmStreamHybrid(child).orElse(tryCollapseFlux(child)).map {
      nativeChild =>
        val (rowOutput, transitionMessage) = boundary match {
          case c2r: ColumnarToRowExecBase =>
            (c2r.withNewChildren(Seq(nativeChild)), "the Gluten C2R remains outside FLUX")
          case _: ColumnarToRowExec =>
            (nativeChild, "Spark C2R removed; FluxNativeQueryExec performs JNI row conversion")
        }
        logWarning(
          "FluxCollapseRule: *** INTENTIONAL TERMINAL NATIVE ROW OUTPUT *** " +
   ...
   * columnar child. Preserve that required transition outside FLUX, but peel it before validating
  ...
  private def collapseTerminalObjectEgress(boundary: DeserializeToObjectExec): Option[SparkPlan] =
    boundary.child match {
      ...
      case c2r: ColumnarToRowExec =>
        tryCollapseJvmStreamHybrid(c2r.child).orElse(tryCollapseFlux(c2r.child)).map {
          nativeChild =>
            logWarning(
              "FluxCollapseRule: *** INTENTIONAL TERMINAL NATIVE OBJECT OUTPUT *** " +
                s"boundary=${boundary.getClass.getSimpleName}; direct Spark C2R removed and " +
                "FluxNativeQueryExec performs the JNI row conversion")
  ...
  private def collapseTerminalObjectEgressThroughC2r(
    ...
    tryCollapseJvmStreamHybrid(relationalChild).orElse(tryCollapseFlux(relationalChild)).map {
      nativeChild =>
        ...
        logWarning(
          "FluxCollapseRule: *** INTENTIONAL TERMINAL NATIVE OBJECT OUTPUT *** " +
            ...
            s"direct ${directRowTransition.getClass.getSimpleName} remain outside FLUX")
  ...
  private def tryCollapseArrowPythonBoundary(plan: SparkPlan): Option[SparkPlan] = {
    ...
    val normalized = normalizeFluxNativeOperators(plan)
    ...
    if (!sawArrowBoundary || failedProducer || !isSupportedArrowHybridPlan(rewritten)) {
      ...
      logWarning(
        "FluxCollapseRule: *** INTENTIONAL COLUMNAR ARROW PYTHON FLUX BOUNDARY *** " +
   ...
   * those conversions inside FluxNativeQueryExec would make FLUX advertise/output its native batch
  ...
  private def collapseArrowProducer(plan: SparkPlan): Option[SparkPlan] = plan match {
    case flux: FluxNativeQueryExec => Some(flux)
    ...
    case _: ColumnarToRowExecBase | _: RowToColumnarExecBase =>
      None
    case nativeProducer =>
      tryCollapseFlux(nativeProducer)
  }
   ...
   * rejected even if normal non-FLUX Gluten would otherwise insert them.
   */
    ...
    plan match {
      ...
      case arrow: ColumnarArrowEvalPythonExec =>
        arrow.child.find {
          ...
          case _ => false
        }.isEmpty && containsFluxProducer(arrow.child)
  ...
  private def containsFluxProducer(plan: SparkPlan): Boolean = plan match {
    case _: FluxNativeQueryExec => true
    case other => other.children.exists(containsFluxProducer)
  }

  /**
   * Attempt to collapse the entire plan into a single FluxNativeQueryExec.
   *
   * Returns None if any part of the plan cannot be handled in FLUX mode (i.e., contains
   * non-TransformSupport operators that are not exchanges).
   */
  private def tryCollapseFlux(plan: SparkPlan): Option[FluxNativeQueryExec] = {
    ...
    // first would also erase an unrelated nested C2R and weaken the strict-FLUX contract.
    val repaired = repairRecoverableRowShells(plan)
    findUnpairedNestedRowOutput(repaired) match {
      case Some(boundary) =>
        logWarning(
          "FluxCollapseRule: refusing to normalize an unpaired nested C2R; " +
    ...
    val normalized = normalizeFluxNativeOperators(repaired)
    tryCollapseNormalizedFlux(
      normalized,
   ...
   * arbitrary nested row consumer. The latter must continue to fail strict FLUX validation.
   */
    ...
    case generate: GenerateExec if GenerateExecTransformer.supportsGenerate(generate.generator) =>
      generate.child match {
        case c2r: ColumnarToRowExecBase if c2r.children.size == 1 =>
          ...
          FluxCollapseRule.sameOutput(c2r.output, nativeChild.output)
        case c2r: ColumnarToRowExec =>
          c2r.child.supportsColumnar &&
          FluxCollapseRule.sameOutput(c2r.output, c2r.child.output)
        case _ => false
      }
    case _ => false
  }

  private def tryCollapseJvmStreamHybrid(plan: SparkPlan): Option[FluxNativeQueryExec] = {
    ...
    // native rewrites.
    val transitionBridged = FluxColumnarTransitionBridge()(repaired)
    ...
    val nativeUnionRewritten = rewriteFluxNativeUnion(nativeSortKeysProjected)
    logHybridIngressTransition(
      "native-union-rewrite",
      nativeSortKeysProjected,
      nativeUnionRewritten)
    val unionRewritten = FluxReplicatedCartesianRule()(nativeUnionRewritten)
    ...
    // consumer from which dynamic FLUX extraction can build one real fragment.
    val nativeAnchored =
  ...
  private def tryCollapseNormalizedFlux(
      unionRewritten: SparkPlan,
      allowJvmStreamIngress: Boolean,
      mode: String): Option[FluxNativeQueryExec] = {
    arrowScalarNormalizationRejection(unionRewritten).foreach {
      reason =>
        if (failOnFallback) {
          throw new IllegalStateException(s"FluxCollapseRule: strict FLUX rejected plan: $reason")
        }
    }
    if (!isNativeSupported(unionRewritten, allowJvmStreamIngress)) {
      ...
      logWarning(
        s"FluxCollapseRule: plan contains non-native operators, " +
          s"cannot collapse to FLUX. First blocker: $reason. " +
          s"Plan root: ${unionRewritten.getClass.getSimpleName}. " +
          s"Plan: ${unionRewritten.treeString.take(500)}")
      return None
    }

    logWarning(s"FluxCollapseRule: plan is $mode, wrapping with FluxNativeQueryExec")
    ...
    // FluxNativeQueryExec.doExecuteColumnar() will delegate to child.executeColumnar()
    // (BSP execution). True FLUX fragment extraction comes in Phase 2.
    val fragments = Seq(
      NativeFragment(
        id = 0,
    ...
    logWarning(
      s"FluxCollapseRule: *** FLUX PLAN ADMITTED *** - operator admission passed; " +
        ...
        s"placeholder and is not an execution topology or an end-to-end fully-FLUX verdict. " +
        s"mode=$mode")

    // Plan D: Wrap, don't replace. Keep original plan as child so Spark's
    // shuffle/broadcast validation passes. At execution time, FluxNativeQueryExec
    // bypasses child.executeColumnar() and runs via FluxQueryCoordinator instead.
    Some(
      FluxNativeQueryExec(
        child = rewritten,
        fragments = fragments,
  ...
  private[extension] def normalizeFluxNativeOperators(plan: SparkPlan): SparkPlan =
    normalizeFluxNativeOperators(plan, preserveJvmStreamIngress = false, recoverGenerate = false)

  private def normalizeFluxNativeOperators(
      plan: SparkPlan,
    ...
    normalizeFluxNativeOperators(
      ...
      preserveJvmStreamIngress,
      recoverGenerate,
      rewriteNativeUnion = rewriteFluxNativeUnion)

  private[extension] def normalizeFluxNativeOperators(
      plan: SparkPlan,
      preserveJvmStreamIngress: Boolean,
  ...
  private def normalizeFluxNativeOperators(
    ...
    // convention expected by an exchange or V2 writer. FLUX absorbs that boundary, so eliminate
    ...
    val transitionBridged = FluxColumnarTransitionBridge()(plan)
    ...
    // FLUX owns peer parallelism and the Iceberg-required exchange immediately
    ...
    val coalesceElided = transitionBridged.transformUp {
      case coalesce: ColumnarCoalesceExec if coalesce.numPartitions > 1 =>
        logInfo(
          s"FluxCollapseRule: eliding non-semantic ColumnarCoalesceExec(" +
            s"${coalesce.numPartitions}) inside FLUX query")
        coalesce.child
    }
    ...
    val adjacentShuffleElided = coalesceElided.transformUp {
      case outer: ShuffleExchangeLike if outer.child.isInstanceOf[ShuffleExchangeLike] =>
        ...
        logInfo(
          s"FluxCollapseRule: eliding redundant adjacent " +
    ...
    // strict-FLUX subtree. This is expression-preserving and does not broaden native validation.
    ...
    // profitability/validation pass declines a very wide aggregate. FLUX has a
    ...
    // and other real row consumers as strict-FLUX negatives.
    ...
    FluxReplicatedCartesianRule()(unionRewritten)
  }
  ...
  private def rewriteFluxNativeUnion(plan: SparkPlan): SparkPlan = {
  ...
  /**
   * Check whether the entire plan tree can be executed natively in FLUX mode.
   *
   ...
   * ordinary strict-FLUX contract. At least one exact ingress must be present; every other node
  ...
  private def logHybridIngressStage(stage: String, plan: SparkPlan): Boolean = {
    ...
    val objectGuardOutcomes = FluxJvmStreamInputMatcher.objectIngressGuardOutcomes(plan)
    val candidateText =
      if (objectGuardOutcomes.isEmpty) "objectIngressCandidates=none"
      else objectGuardOutcomes.mkString("objectIngressCandidates=[", " | ", "]")
    val message =
      s"FluxCollapseRule: exact JVM stream ingress diagnostic: stage=$stage; " +
  ...
  private def logHybridIngressTransition(
    ...
    if (beforeHasIngress && !afterHasIngress) {
      logWarning(
        s"FluxCollapseRule: exact JVM stream ingress LOST after hybrid normalization stage=$stage")
    }
  }

  private def isExactJvmStreamIngress(plan: SparkPlan): Boolean = plan match {
    case r2c: RowToColumnarExecBase => FluxJvmStreamInputMatcher.rowInput(r2c).isDefined
    case r2c: RowToColumnarExec => FluxJvmStreamInputMatcher.rowInput(r2c).isDefined
    case cache: InMemoryTableScanExec =>
      FluxJvmStreamInputMatcher.rowInput(cache).isDefined
    case _ => false
  }

  private def isNativeSupported(plan: SparkPlan, allowJvmStreamIngress: Boolean): Boolean = {
    plan match {
      // Exchanges that we can absorb into the FLUX plan
      case exchange: ShuffleExchangeLike =>
      ...
      // The native FLUX runtime treats BROADCAST as a distinct exchange type.
      case bc: BroadcastExchangeLike =>
        ...
        if (!absorbable) {
          logWarning(
            s"FluxCollapseRule: BLOCKED by BroadcastExchangeLike: " +
      ...
      case python: EvalPythonExecTransformer
        ...
        logWarning(
          s"FluxCollapseRule: BLOCKED by unsafe ordinary Arrow scalar normalization: " +
      ...
      // it: FluxNativeQueryExec captures and columnarizes the row plan as a local stream.
      case r2c: RowToColumnarExecBase
          if allowJvmStreamIngress && FluxJvmStreamInputMatcher.rowInput(r2c).isDefined =>
        true
      case r2c: RowToColumnarExec
          if allowJvmStreamIngress && FluxJvmStreamInputMatcher.rowInput(r2c).isDefined =>
        true
      case cache: InMemoryTableScanExec
          if allowJvmStreamIngress && FluxJvmStreamInputMatcher.rowInput(cache).isDefined =>
        true

      // A remaining row transition is a real execution boundary. The only C2R shape that FLUX may
      ...
      case c2r: ColumnarToRowExecBase =>
        logWarning(
          s"FluxCollapseRule: BLOCKED by native-to-row execution boundary: " +
      ...
      case r2c: RowToColumnarExecBase =>
        logWarning(
          s"FluxCollapseRule: BLOCKED by row-to-native execution boundary: " +
      ...
      // InputIteratorTransformer at exchange boundaries. It is transparent to FLUX fragment
      ...
      // Whitelist these so the outer plan can collapse to FLUX.
      case f: FilterExec if containsScalarSubquery(f.condition) =>
        isNativeSupported(f.child, allowJvmStreamIngress)
      case p: ProjectExec if p.projectList.exists(containsScalarSubquery) =>
        isNativeSupported(p.child, allowJvmStreamIngress)

      // Write commands are planned before FluxNativeQueryExec gets a chance to run its
      ...
      // FluxNativeQueryExec.offloadLocalHashJoins converts these exact nodes to their transformer
      ...
      // Any other non-native operator means we cannot do FLUX
      case other =>
        logWarning(
          s"FluxCollapseRule: BLOCKED by non-native operator: " +
   ...
   * is still rejected by strict FLUX.
   */
  ...
  /**
   * Check whether an exchange can be absorbed into the FLUX plan.
   *
    ...
    exchange match {
      case shuffle: ShuffleExchangeLike =>
        ...
        val childSupported = shuffle.child.isInstanceOf[TransformSupport] ||
          ...
          // that shape so normalizeFluxNativeOperators can remove the redundant
        ...
        if (!partitioningSupported) {
          logWarning(
            s"FluxCollapseRule: canAbsorbExchange=false: unsupported partitioning " +
              s"${shuffle.outputPartitioning.getClass.getSimpleName}")
        }
        if (!childSupported) {
          logWarning(
            s"FluxCollapseRule: canAbsorbExchange=false: child not supported: " +
  ...
  /** Check whether a partitioning scheme is supported for FLUX streaming exchange. */
  private def isSupportedPartitioning(partitioning: Partitioning): Boolean = {
    partitioning match {
      ...
      case other =>
        logWarning(
          s"FluxCollapseRule: unsupported partitioning type: ${other.getClass.getSimpleName}")
        false
    }
  }
  ...
  private def extractFragments(
     ...
     * computes Murmur3Hash. In FLUX, we recompute that hash inside Velox PartitionedOutputNode's
      ...
      node match {
        case shuffle: ShuffleExchangeLike =>
          ...
          if (isSingleGather && collapseSingleGather) {
            logWarning(
              "FluxCollapseRule.walk: collapsing SinglePartition shuffle into parent fragment " +
          ...
          // VeloxSparkPlanExecApi injects: in FLUX we recompute the hash on-the-fly
          ...
          val exchangeSource = FluxExchangeSourceTransformer(
            exchangeCounter.get(),
            shuffle.output
          )
        ...
        case broadcast: BroadcastExchangeLike =>
          ...
          rejectUnsafeBroadcastFusionIfRequested("FluxCollapseRule")
  ...
  private def classifyPartitioning(partitioning: Partitioning): (String, Seq[Attribute]) = {
    partitioning match {
      ...
      case other =>
        logWarning(
          s"FluxCollapseRule: unexpected partitioning type: ${other.getClass.getSimpleName}")
        ("HASH", Seq.empty)
    }
  }
