 ...
 * Spark's TakeOrderedAndProject hides that split inside one root operator, so the MPP fragment walk
 ...
 */
case class MppRootTopNPartialRule() extends Rule[SparkPlan] with Logging {
    ...
    case topk: TakeOrderedAndProjectExecTransformer if topk.offset == 0 =>
      buildPartialTopN(topk.limit, topk.sortOrder, topk.child) match {
        case Some(rewrittenChild) =>
          logWarning(
            s"MppRootTopNPartialRule: inserted partial TopN(limit=${topk.limit}) " +
  ...
  private def buildPartialTopN(
    ...
    if (validation.ok()) {
      ...
      logWarning(
        s"MppRootTopNPartialRule: TopN validation failed ($validation); leaving plan unchanged")
      None
    }
  }
