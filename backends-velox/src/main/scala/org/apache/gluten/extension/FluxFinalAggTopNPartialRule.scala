 ...
 */
case class FluxFinalAggTopNPartialRule() extends Rule[SparkPlan] with Logging {
  ...
  private def rewriteAll(node: SparkPlan): SparkPlan = node match {
    ...
    case topk: TakeOrderedAndProjectExecTransformer if topk.offset == 0 =>
      ...
      if (rewrittenChild.fastEquals(topk.child)) {
        ...
        logInfo(
          s"FluxFinalAggTopNPartialRule: inserted partial TopN(limit=${topk.limit}) " +
  ...
  private def buildPartialTopN(
    ...
    if (missingReferences.nonEmpty) {
      logWarning(
        s"FluxFinalAggTopNPartialRule: sort references ${missingReferences.mkString(",")} " +
    ...
    if (validation.ok()) {
      ...
      logWarning(
        s"FluxFinalAggTopNPartialRule: TopN validation failed ($validation); " +
