...
private[gluten] object MppBroadcastLifecycle {
   ...
   *
   * This helper is deliberately shared by the final planning rule and MppNativeQueryExec's
  ...
  def deferBroadcastPreparation(plan: SparkPlan): Int = {
    ...
    def visit(node: SparkPlan): Unit = {
      ...
      node match {
        case prepared: MppPreparedChildExec => visit(prepared.hiddenPlan)
        ...
        case broadcast: ColumnarBroadcastExchangeExec =>
          if (broadcast.deferPrepareForMppNativeExecution()) {
            marked += 1
          }
...
/**
 * Defer eager broadcast preparation until native MPP execution is committed.
 *
 * Spark calls `prepare()` before `MppNativeQueryExec` reaches its runtime validation point. Leaving
 * a broadcast untouched here starts its relation future eagerly, so a native MPP query runs an
 ...
 *
 * [[MppNativeQueryExec]] still owns irreversible suppression. RANGE sampling and BSP fallback can
 * call `doExecuteBroadcast`, which starts a deferred exchange lazily. Once all fallback-capable
 * validation succeeds, native MPP calls `suppressDeadBroadcastsForNativeMpp` and permanently fails
 * any dead relation promise.
 *
  ...
  override def apply(plan: SparkPlan): SparkPlan = {
    ...
    plan.foreach {
      case mpp: MppNativeQueryExec =>
        markedCount += MppBroadcastLifecycle.deferBroadcastPreparation(mpp.child)
      case _ =>
    }

    if (markedCount > 0) {
      logDebug(
        s"MppSuppressDeadBroadcastsRule: deferred eager prepare for $markedCount " +
