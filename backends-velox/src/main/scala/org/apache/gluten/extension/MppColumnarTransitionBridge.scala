 ...
 * when MPP absorbs that boundary and keeps the data columnar end to end. Executing those adapters
 ...
 * It also removes an identity C2R directly below a Gluten columnar shuffle because MPP absorbs the
 ...
 */
case class MppColumnarTransitionBridge() extends Rule[SparkPlan] with Logging {
  ...
  private def bridge(r2c: SparkPlan, c2r: SparkPlan, columnarChild: SparkPlan): SparkPlan = {
    if (
      ...
      logInfo(
        s"MppColumnarTransitionBridge: removed redundant " +
  ...
  private def bridgeExchangeInput(
    ...
    if (
      ...
      logInfo(
        s"MppColumnarTransitionBridge: removed identity ${c2r.getClass.getSimpleName} " +
