 ...
 * only when Catalyst statistics prove that one complete input is bounded by the configured FLUX
 * build-side limit.
 *
 * A plain [[CartesianProductExecTransformer]] cannot be fused into an FLUX fragment: Spark's
 ...
 * Spark partition-pair implementation remains available. Strict FLUX fails during planning with a
 * targeted validation error instead of running an incomplete local Cartesian product.
 */
case class FluxReplicatedCartesianRule() extends Rule[SparkPlan] with Logging {

  import FluxReplicatedCartesianRule._
      ...
      case cart @ CartesianProductExecTransformer(
        ...
        chooseBuildSide(leftStats, rightStats, maxBuildBytes) match {
          ...
          case Some(BuildLeft) =>
            ...
            // so FLUX iterator slots match Substrait source order. Restore the original Spark
          ...
          case None =>
            ...
            if (strictFlux) {
              throw new IllegalStateException(
                s"FluxReplicatedCartesianRule: unsafe replicated Cartesian: $reason. " +
            ...
            logWarning(s"FluxReplicatedCartesianRule: keeping Spark Cartesian bridge: $reason")
            cart
        }
    }
  }
  ...
  private def strictFlux: Boolean =
  ...
  private def logRewrite(
    ...
    logInfo(
      s"FluxReplicatedCartesianRule: replacing ${cart.nodeName} with native replicated " +
  ...
  private def markReplicatedCartesian(join: VeloxBroadcastNestedLoopJoinExecTransformer)
      : VeloxBroadcastNestedLoopJoinExecTransformer = {
    // Carry the exact, already canonicalized byte count with this physical query. FLUX forwards the
...
object FluxReplicatedCartesianRule {
  ...
  val REPLICATED_CARTESIAN_MAX_BUILD_BYTES_TAG: TreeNodeTag[Long] =
    TreeNodeTag[Long]("gluten.flux.replicatedCartesian.maxBuildBytes")
