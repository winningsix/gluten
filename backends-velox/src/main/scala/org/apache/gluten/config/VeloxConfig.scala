...
object VeloxConfig extends ConfigRegistry {
  ...
  val FLUX_SINGLE_TASK_MODE =
    buildConf("spark.gluten.sql.columnar.backend.velox.flux.singleTaskMode")
      .doc(
        "Collapse local-mode FLUX fragments into one Velox task connected by local exchanges, " +
          "bypassing UcxExchange for single-worker execution.")
      .booleanConf
      .createWithDefault(true)

  val FLUX_SINGLE_TASK_MAX_DRIVERS =
    buildConf("spark.gluten.sql.columnar.backend.velox.flux.singleTaskMaxDrivers")
      .doc("Maximum driver count for the merged Velox task when FLUX single-task mode is enabled.")
      .intConf
      .createWithDefault(2)
  ...
  val CUDF_HIVE_SELECTIVE_PRELOAD_ENABLED =
    buildStaticConf("spark.gluten.sql.columnar.backend.velox.cudf.hive.selectivePreloadEnabled")
      .doc(
  ...
  val CUDF_HIVE_PREFETCH_MAX_INFLIGHT_BYTES =
    buildStaticConf("spark.gluten.sql.columnar.backend.velox.cudf.hive.prefetchMaxInFlightBytes")
      .doc("Maximum Parquet range bytes admitted concurrently by the executor-wide " +
        "selective prefetch broker.")
      .bytesConf(ByteUnit.BYTE)
      .createWithDefault(4L * 1024 * 1024 * 1024)

  val CUDF_HIVE_PREFETCH_THREADS =
    buildStaticConf("spark.gluten.sql.columnar.backend.velox.cudf.hive.prefetchThreads")
      .doc("Number of CPU range-read workers in the executor-wide selective prefetch broker.")
      .intConf
  ...
  val CUDF_GPU_MEMORY_SIZE =
    buildConf("spark.gluten.sql.columnar.backend.velox.cudf.gpuMemorySize")
      .doc(
        "Total GPU memory available for task scheduling (in bytes). " +
          "If not set, defaults to the GPU memory allocated by RMM.")
      .bytesConf(ByteUnit.BYTE)
      .createOptional
