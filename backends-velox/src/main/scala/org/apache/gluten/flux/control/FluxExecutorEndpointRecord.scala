 ...
 *     `FluxNativeQueryExec.fluxPeerInfos()` (do not break this without a coordinated native change;
 *     native JNI consumers depend on it).
 */
case class FluxExecutorEndpointRecord(
    executorId: String,
    host: String,
  ...
  def isLive: Boolean = state == FluxExecutorEndpointRecord.StateLive
}

object FluxExecutorEndpointRecord {
