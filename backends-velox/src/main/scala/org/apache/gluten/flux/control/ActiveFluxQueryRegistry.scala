...
/** Executor-local key for the Spark task attempt that owns one native FLUX peer handle. */
final private[control] case class ActiveFluxQueryKey(
    runId: FluxQueryRunId,
    peerIndex: Int,
    taskAttemptId: Long)

/**
 * Executor-local lifecycle holder for one native FLUX peer.
 *
 ...
 * or the native handle; `FluxNativeQueryRDD` remains the sole exactly-once close owner.
 */
final private[gluten] class ActiveFluxQuery private[control] (
  ...
  private val currentState = new AtomicReference[String](FluxPeerState.Created)
  ...
  private[control] val key = ActiveFluxQueryKey(runId, peerIndex, taskAttemptId)

  /** Task-thread gate called immediately before nativeStartFluxQuery. */
  def beginNativeStart(): Boolean = synchronized {
    if (abortRequestedFlag.get() || taskContext.isInterrupted()) {
      abortRequestedFlag.set(true)
      currentState.set(FluxPeerState.AbortRequested)
      startFinished.countDown()
      false
    } else {
      currentState.set(FluxPeerState.Starting)
      true
    }
  }
  ...
  def finishNativeStart(succeeded: Boolean): Unit = synchronized {
    ...
    if (succeeded && !abortRequestedFlag.get()) {
      currentState.set(FluxPeerState.Running)
  ...
  private[control] def markOutputComplete(): Boolean = synchronized {
    if (abortRequestedFlag.get() || isTerminal || taskContext.isInterrupted()) {
      ...
      currentState.set(FluxPeerState.OutputComplete)
      true
    }
  }
  ...
  private[control] def snapshot(): FluxPeerSnapshot =
    FluxPeerSnapshot(
      runId,
      peerIndex,
      expectedPeerCount,
  ...
  def isTerminal: Boolean = FluxPeerState.isTerminal(currentState.get())
 ...
 */
private[control] class ActiveFluxQueryRegistry {
  private val entries = new ConcurrentHashMap[ActiveFluxQueryKey, ActiveFluxQuery]()

  def register(query: ActiveFluxQuery): Unit = {
    val previous = entries.putIfAbsent(query.key, query)
    if (previous != null) {
      throw new IllegalStateException(
        s"FLUX query already registered: run=${query.runId.logId} peer=${query.peerIndex} " +
          s"taskAttempt=${query.taskAttemptId}")
    }
  }

  def find(command: FluxAbortQuery): Option[ActiveFluxQuery] =
    Option(entries.get(ActiveFluxQueryKey(command.runId, command.peerIndex, command.taskAttemptId)))
      .filter {
        query =>
  ...
  def find(completion: FluxPeerCompletion): Option[ActiveFluxQuery] =
    Option(
      entries.get(
        ActiveFluxQueryKey(completion.runId, completion.peerIndex, completion.taskAttemptId)))
      .filter {
        query =>
  ...
  // must not serialize a Stream closure that still captures the non-serializable ActiveFluxQuery.
  def values: Seq[ActiveFluxQuery] = entries.values().asScala.iterator.toVector

  def remove(query: ActiveFluxQuery): Unit = entries.remove(query.key, query)
 ...
 */
private[control] class GlutenFluxExecutorControlAgent(
    val executorId: String,
    val executorSessionId: String,
  ...
  private val registry = new ActiveFluxQueryRegistry
  private val pendingFailures = new ConcurrentHashMap[String, FluxQueryFailure]()
  private val pendingTerminals = new ConcurrentHashMap[String, FluxTerminalEvent]()
  ...
  private val abortWorker =
    Executors.newSingleThreadExecutor(threadFactory("gluten-flux-abort-worker"))

  if (startThreads) {
    interruptWatcher =
      Executors.newSingleThreadScheduledExecutor(threadFactory("gluten-flux-interrupt-watcher"))
    heartbeatPoller =
      Executors.newSingleThreadScheduledExecutor(threadFactory("gluten-flux-heartbeat-poller"))
  ...
  def register(
      ...
      abortNative: () => Unit): ActiveFluxQuery = {
    val query = new ActiveFluxQuery(
      runId,
      peerIndex,
      expectedPeerCount,
  ...
  def awaitPeerCompletion(query: ActiveFluxQuery): Boolean = {
    ...
    try {
      ...
      case _: InterruptedException =>
        val reason = "FLUX peer completion wait interrupted"
  ...
  private[control] def sendHeartbeat(): Unit = synchronized {
    ...
    val heartbeat = FluxQueryHeartbeat(
      executorId,
      executorSessionId,
      clock(),
    ...
    askDriver(heartbeat) match {
      case ack: FluxQueryHeartbeatAck =>
      ...
      case other =>
        logWarning(s"Unexpected FLUX query heartbeat response: $other")
    }
  }

  private def enqueueAbort(query: ActiveFluxQuery, sequence: Long, reason: String): Unit = {
    if (query.requestAbort(sequence, reason)) {
      logWarning(
        s"Queueing FLUX abort run=${query.runId.logId} peer=${query.peerIndex} " +
          s"taskAttempt=${query.taskAttemptId} sequence=$sequence reason=$reason")
      abortWorker.execute(new Runnable {
        override def run(): Unit = {
          try {
            logWarning(
              s"Starting native FLUX abort run=${query.runId.logId} peer=${query.peerIndex}")
            query.runNativeAbort()
            logWarning(
              s"Native FLUX abort returned run=${query.runId.logId} peer=${query.peerIndex}")
          } catch {
            case NonFatal(e) =>
              logWarning(
                s"Native FLUX abort failed run=${query.runId.logId} peer=${query.peerIndex}",
                e)
          }
        }
      })
  ...
  private[control] def activeQueries: Seq[ActiveFluxQuery] = registry.values
  ...
  private def failureEventId(query: ActiveFluxQuery): String =
    s"${query.runId.logId}/${query.peerIndex}/${query.taskAttemptId}/failure"

  private def terminalEventId(query: ActiveFluxQuery): String =
  ...
  private def safely(label: String)(body: => Unit): Unit = {
    ...
    try {
      ...
      case NonFatal(e) if isHeartbeat && heartbeatFailureLogged.compareAndSet(false, true) =>
        logWarning(s"FLUX query-control $label failed; retries will continue", e)
      case NonFatal(e) => logDebug(s"FLUX query-control $label failed", e)
    }
  }
...
private[control] object GlutenFluxExecutorControlAgent {
  def apply(
      executorId: String,
      askDriver: Any => AnyRef,
      interruptPollMs: Long,
      heartbeatMs: Long): GlutenFluxExecutorControlAgent =
    new GlutenFluxExecutorControlAgent(
      executorId,
