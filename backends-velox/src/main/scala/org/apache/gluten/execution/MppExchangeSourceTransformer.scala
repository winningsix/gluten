...
/**
 * Replaces [[InputIteratorTransformer]] at MPP fragment boundaries.
 *
 ...
 *
 * In MPP mode, this transformer generates a `ReadRel` with a special tag indicating
 * "mpp-exchange-source". The C++ side will create a `GpuExchange` operator that reads from the
 ...
 */
case class MppExchangeSourceTransformer(
    exchangeId: Int,
  ...
  override def nodeName: String = s"MppExchangeSource(exchange=$exchangeId)"

  override def simpleString(maxFields: Int): String = {
    s"MppExchangeSource(exchange=$exchangeId, " +
  ...
  override protected def doTransform(context: SubstraitContext): TransformContext = {
    ...
    // The native side identifies this as an MPP exchange source via the exchangeId
    // property carried in the Substrait plan extension.
    // TODO: Add advanced extension tag/property to the ReadRel to mark as
    //       "mpp-exchange-source" with exchangeId, so the C++ side creates
