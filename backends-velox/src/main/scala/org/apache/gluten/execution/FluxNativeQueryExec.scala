...
import org.apache.gluten.vectorized.FluxQueryJniWrapper
...
/**
 * A single SparkPlan node that represents the entire FLUX query execution.
 *
 ...
 *   1. Serialize all fragments' Substrait plans and exchange specs into a single FLUX execution
 ...
 * @param originalPlan
 *   The original SparkPlan before FLUX collapse (for explain/debugging).
 */
/**
 * Plan D: Wrap, don't replace.
 *
 * FluxNativeQueryExec wraps the original plan as its child (UnaryExecNode). Spark sees the original
 ...
 * the child plan only to extract Substrait fragments, then executes via JNI -> FluxQueryCoordinator
 * (streaming exchange).
 */
/**
...
private[execution] case class FluxLocalStreamInput(
    fragmentId: Int,
    slotIdx: Int,
    rdd: RDD[ColumnarBatch])

private[execution] case class FluxLocalStreamSlot(fragmentId: Int, slotIdx: Int)
 ...
 */
private[execution] object FluxRangeTopology {
      ...
      case spec if spec.exchangeType == "RANGE" =>
        val effective = spec.rangeEffectivePartitions.getOrElse {
          throw new IllegalStateException(
            s"FLUX RANGE exchange ${spec.id} has no effective partition count")
        }
        require(
          effective > 0,
          s"FLUX RANGE exchange ${spec.id} has invalid effective partition count $effective")
        require(
          effective <= spec.numPartitions,
          s"FLUX RANGE exchange ${spec.id} effective partitions $effective exceed requested " +
 ...
 */
private[execution] object FluxHashJoinInputSortRewrite {
...
/**
 * Fail-closed matcher for the JVM-backed stream shapes admitted by strict FLUX.
 *
 * FLUX already feeds a Spark `RDDScanExec` into its existing local `InputIterator` bridge. This
...
private[gluten] object FluxJvmStreamInputMatcher {
   ...
   * each guard outcome explicit makes a strict-FLUX rejection actionable while preserving the
  ...
  private def directExternalObjectScan(plan: SparkPlan): Option[ExternalRDDScanExec[_]] =
    plan match {
      ...
      // admit arbitrary row subtrees as strict-FLUX local streams. ColumnarInputAdapter is a
...
private case class FluxReplicatedJoinBuildInput(child: SparkPlan) extends UnaryTransformSupport {
  ...
  override def nodeName: String = "FluxReplicatedJoinBuildInput"

  override protected def doTransform(context: SubstraitContext): TransformContext = {
    // Plan-D write commands can reach this FLUX-only marker before Gluten's normal columnar
...
private[execution] object FluxBspFallbackPreparation {

  /**
   * Remove metadata-only nodes introduced while shaping the native FLUX graph. They deliberately
  ...
  def stripNativeOnlyMarkers(plan: SparkPlan): SparkPlan = {
    plan.transformUp { case marker: FluxReplicatedJoinBuildInput => marker.child }
  }
  ...
  private[execution] def replicatedJoinBuildMarkerForTests(child: SparkPlan): SparkPlan = {
    FluxReplicatedJoinBuildInput(child)
  }
}

private case class FluxPartitioningPreservingWrapper(
    child: SparkPlan,
  ...
  override def nodeName: String = s"FluxPartitioningPreservingWrapper(exchange=$exchangeId)"
...
private[gluten] case class FluxPreparedChildExec(hiddenPlan: SparkPlan)
  ...
  override protected def doExecute(): RDD[InternalRow] =
    throw new UnsupportedOperationException("FluxPreparedChildExec is only a prepare-time wrapper")

  override protected def doExecuteColumnar(): RDD[ColumnarBatch] =
    throw new UnsupportedOperationException("FluxPreparedChildExec is only a prepare-time wrapper")
}

case class FluxNativeQueryExec(
    child: SparkPlan,
    fragments: Seq[NativeFragment],
    exchanges: Seq[ExchangeSpec],
  ...
  private def preparedChildPlan: SparkPlan = unwrapPreparedFluxChild(child)
  ...
  override def batchType(): Convention.BatchType = {
    // FLUX execution produces native columnar batches
    org.apache.gluten.backendsapi.BackendsApiManager.getSettings.primaryBatchType
  }

  override def rowType0(): Convention.RowType = Convention.RowType.None

  // Must match children.size (1 child = FluxSchemaOnlyExec or planLater)
  override def requiredChildConvention(): Seq[ConventionReq] = {
    children.map(_ => ConventionReq.any)
  }

  private val FLUX_PEER_ENDPOINTS_KEY = "spark.gluten.mpp.peerEndpoints"
  ...
  private val FLUX_SINGLE_TASK_MODE_KEY =
    "spark.gluten.sql.columnar.backend.velox.flux.singleTaskMode"
  ...
  private def isExplicitSingleTaskMode: Boolean =
    SQLConf.get.getConfString(FLUX_SINGLE_TASK_MODE_KEY, "false").toBoolean
  ...
  private def scalarSubqueryRewriteDisabledReason: String = {
    ...
    if (
      ...
      s"$FLUX_SINGLE_TASK_MODE_KEY=true"
    } else {
      s"$SCALAR_SUBQUERY_REWRITE_ENABLED_KEY=false"
    }
  }

  private def newNativeFluxQueryId(): String = {
  ...
  override def doPrepare(): Unit = {
    // FLUX bypasses child.executeColumnar(), so preparing the physical child tree here is both
    ...
    logDebug("FluxNativeQueryExec: skipping child prepare; FLUX will prepare required inputs")
  }
  ...
  private def executeColumnarInternal(keepDeviceOutput: Boolean): RDD[ColumnarBatch] = {
    ...
    val deferredBroadcastCount =
      FluxBroadcastLifecycle.deferBroadcastPreparation(executionChild)
    if (deferredBroadcastCount > 0) {
      logDebug(
        s"FluxNativeQueryExec: execution-entry deferred $deferredBroadcastCount newly reachable " +
          "ColumnarBroadcastExchangeExec node(s)")
    }
    // Scope generated RANGE bounds to this FLUX launch. A new action gets a new cache even if it
    // reuses the same SparkPlan object; equivalent exchanges inside this launch join one
    // generation.
    val rangeBoundsCache = new FluxRangeBoundsGenerator.QueryCache(
    ...
    // driver-side scalar materialization. Plan D applies this rewrite earlier in FluxCollapseRule
    // so
    ...
    val childForFlux =
      if (isScalarSubqueryRewriteEnabled) {
        ...
        logInfo(
          s"FluxNativeQueryExec: keeping ScalarSubquery expressions materialized by Spark " +
    ...
    // because FluxNativeQueryExec bypasses child.executeColumnar() we drive it explicitly here.
    ...
    // Phase 2: replace with real FLUX multi-fragment streaming execution.
    ...
    if (!hasRealFragments) {
      ...
      logDebug(s"FluxNativeQueryExec: child plan tree:\n${childForFlux.treeString.take(2000)}")

      val extracted: Either[String, FragmentExtractionResult] =
        try {
          Right(extractFragmentsFromChildPlan(childForFlux))
        } catch {
          case NonFatal(e) =>
            logWarning("FluxNativeQueryExec: FLUX fragment extraction failed", e)
            Left(
      ...
      logDebug(
        s"FluxNativeQueryExec: Phase 2 extracted ${extractedFragments.size} fragments " +
      ...
      logWarning(
        s"FluxNativeQueryExec: *** FLUX EXECUTION TOPOLOGY EXTRACTED *** " +
      ...
      nativeFluxFallbackReason(extractedFragments, extractedExchanges).foreach {
        reason =>
          return delegateToBsp(
            executionChild,
            s"FluxNativeQueryExec: delegating to BSP because extracted FLUX plan is " +
              s"unsafe: $reason")
      }

      val preparedExtractedExchanges =
        try {
          FluxRangeTopology.applyEffectivePartitionCounts(
            prepareHybridFluxRangeExchanges(extractedExchanges, rangeBoundsCache))
        } catch {
          case NonFatal(e) =>
            return delegateToBsp(
              executionChild,
          ...
          case NonFatal(e) =>
            extractedFragments.foreach {
              frag =>
                ...
                logWarning(
                  s"FluxNativeQueryExec: Substrait generation failed while extracted " +
                    s"fragment ${frag.id} root=$rootName " +
      ...
      suppressDeadBroadcastsForNativeFlux(childForFlux)
      suppressDeadBroadcastsForNativeFlux(extractedPlan)

      logDebug(
        s"FluxNativeQueryExec: *** PHASE 3 FLUX EXECUTION *** " +
      ...
      val fluxQueryId = newNativeFluxQueryId()
      ...
      val requestedSparkPartitionCount = fluxSparkPartitionCount
      val sparkPartitionCount =
        effectiveFluxSparkPartitionCount(
          requestedSparkPartitionCount,
          preparedExtractedExchanges,
          childForFlux.find(_.isInstanceOf[WriteFilesExecTransformer]).isDefined)
      val peerResolution = resolveFluxPeers(sparkPartitionCount)
      ...
      val fluxRdd = new FluxNativeQueryRDD(
        sparkContext,
        fragmentPlans,
        numDriversPerFragment,
        exchangeSpecsJson,
        ...
        fragmentSplitInfos,
        fusedBroadcastsByConsumer,
        alignedLocalStreamInputs.map(input => FluxLocalStreamSlot(input.fragmentId, input.slotIdx)),
    ...
    nativeFluxFallbackReason(fragments, exchanges).foreach {
      reason =>
        return delegateToBsp(
          executionChild,
          "FluxNativeQueryExec: delegating to BSP because pre-extracted FLUX plan is unsafe: " +
            reason)
    }
    ...
    val preparedExchanges =
      try {
        FluxRangeTopology.applyEffectivePartitionCounts(
          prepareHybridFluxRangeExchanges(exchanges, rangeBoundsCache))
      } catch {
        case NonFatal(e) =>
          return delegateToBsp(
            executionChild,
    ...
    val replicatedCartesianMaxBuildBytes = replicatedCartesianBuildLimit(childForFlux)

    logDebug(s"FluxNativeQueryExec: generated ${fragmentPlans.length} Substrait plans on driver")
    ...
    // Pre-built fragments path doesn't fuse broadcasts (legacy FluxCollapseRule
  ...
  private def replicatedCartesianBuildLimit(plan: SparkPlan): Long = {
    ...
    require(
      limits.size <= 1,
      s"FLUX query has inconsistent replicated-Cartesian build limits: ${limits.mkString(", ")}")
    limits.headOption.getOrElse(0L)
  }

  // --- Explain / toString ---

  override def nodeName: String = s"FluxNativeQuery"

  override def simpleString(maxFields: Int): String = {
    s"FluxNativeQuery(${fragments.size} fragments, ${exchanges.size} exchanges)"
  }

  override def verboseStringWithOperatorId(): String = {
    val sb = new StringBuilder
    sb.append(s"FluxNativeQuery (${fragments.size} fragments, ${exchanges.size} exchanges)\n")
    sb.append(fragmentSummary)
    sb.append(exchangeSummary)
    sb.toString()
  }

  override protected def withNewChildInternal(newChild: SparkPlan): FluxNativeQueryExec = {
    copy(child = maybeHidePreparedFluxChild(newChild), originalLogicalPlan = originalLogicalPlan)
  }
  ...
  private def prepareColumnarBspFallbackPlan(plan: SparkPlan): SparkPlan = {
    // Cross-cut FLUX rewrites annotate replicated build inputs so fragment extraction can force
    ...
    val executablePlan = FluxBspFallbackPreparation.stripNativeOnlyMarkers(plan)
    val withTransitions =
      InsertTransitions
  ...
  private def maybeHidePreparedFluxChild(plan: SparkPlan): SparkPlan = {
    plan match {
      case _: FluxPreparedChildExec => plan
      case other if containsGlutenColumnarPlan(other) =>
        FluxPreparedChildExec(other)
      case other =>
        other
    }
  }

  private def unwrapPreparedFluxChild(plan: SparkPlan): SparkPlan = {
    plan match {
      case prepared: FluxPreparedChildExec => prepared.hiddenPlan
      case other => other
    }
  }
  ...
  private def containsGlutenColumnarPlan(plan: SparkPlan): Boolean = {
    plan match {
      case _: FluxPreparedChildExec => true
  ...
  private def extractFragmentsFromChildPlan(plan: SparkPlan): FragmentExtractionResult = {
    ...
    val localStreamInputs = mutable.ArrayBuffer[FluxLocalStreamInput]()
    ...
    def ensureWholeStageFragmentRoot(node: SparkPlan): SparkPlan = {
      node match {
        ...
        case ts: TransformSupport if !ts.isInstanceOf[InputIteratorTransformer] =>
          ...
          logInfo(
            s"FluxNativeQueryExec: wrapping raw ${ts.getClass.getSimpleName} exchange producer " +
    ...
    def alignProducerOutputToConsumer(
      ...
      if (fragmentIndex < 0) {
        logWarning(
          s"FluxNativeQueryExec: cannot align missing producer fragment $producerFragmentId")
        return
      }
      ...
      if (projectedOutput.exists(_.isEmpty)) {
        logWarning(
          s"FluxNativeQueryExec: cannot align producer fragment $producerFragmentId output " +
      ...
      fragment.rootOperator match {
        case wst: WholeStageTransformer =>
          ...
          logInfo(
            s"FluxNativeQueryExec: projected exchange producer F$producerFragmentId from " +
              s"${describeAttributes(producerOutput)} to consumed columns " +
              s"${describeAttributes(selected)}")
        case other =>
          logWarning(
            s"FluxNativeQueryExec: cannot project producer fragment $producerFragmentId root " +
    ...
    def walk(plan: SparkPlan): Int = {
      plan match {
        case wst: WholeStageTransformer =>
          ...
          // the remaining schema is exactly the FLUX query output; producer-side hash projects are
              ...
              unwrapToExchange(child) match {
                case Some(bex: BroadcastExchangeLike) if producerFragId < 0 =>
                  ...
                  try {
                    ...
                    logDebug(
                      s"FluxNativeQueryExec: captured fused broadcast at " +
                        s"consumerF=$fragId slot=$slotIdx " +
                        s"(${bex.getClass.getSimpleName})")
                  } catch {
                    case t: Throwable =>
                      logWarning(
                        s"FluxNativeQueryExec: failed to capture broadcast " +
                ...
                case None if producerFragId < 0 && isJvmBackedStreamInput(child) =>
                  localStreamInputs += FluxLocalStreamInput(
                    fragmentId = fragId,
                    slotIdx = slotIdx,
                    rdd = executeJvmBackedStreamColumnar(child))
                  logInfo(
                    s"FluxNativeQueryExec: captured local JVM-backed stream at " +
                      s"consumerF=$fragId slot=$slotIdx")
                ...
                case Some(exchangeNode) =>
                  ...
                  val (exchangeType, partitionKeys, numPartitions) =
                    if (forceBroadcast) {
                      logInfo(
                        s"FluxNativeQueryExec: forcing exchange ${exchangeNode.id} " +
                          s"(${exchangeNode.getClass.getSimpleName}) to BROADCAST for " +
                          s"replicated FLUX hash join build side")
                ...
                case None if unwrapToTopN(child).isDefined =>
                  ...
                  // blocking write-in-FLUX for top-N (LIMIT) queries (Q2/3/10/18/21).
                  extractedExchanges += ExchangeSpec(
                    id = exchangeCounter.getAndIncrement(),
                    producerFragmentId = producerFragId,
                    consumerFragmentId = fragId,
                ...
                case None =>
                  throw new IllegalStateException(
                    s"FluxNativeQueryExec: WST fragment $fragId input slot $slotIdx " +
        ...
        case marker: FluxReplicatedJoinBuildInput =>
          walk(marker.child)
        ...
        case exchange: ShuffleExchangeLike =>
          ...
          if (isSingleGather && collapseSingleGather) {
            logInfo(
              "FluxNativeQueryExec.walk: collapsing SinglePartition shuffle into parent fragment")
            walk(unwrapTransparent(exchange.child))
            -1
          } else {
            // Exchange boundary: walk the producer side for this consumer.
            //
            // Native FLUX emits exactly one PartitionedOutputNode for each fragment.
            ...
            // (mirrors FluxCollapseRule.stripSyntheticHashProject) so the producer
            ...
            // (FluxJniWrapper.cc), and Velox's PartitionedOutputNode partitions BEFORE
        ...
        case bex: BroadcastExchangeLike =>
          ...
          // shared one JNI iterator across fanout drivers and is not safe for FLUX.
          rejectUnsafeBroadcastFusionIfRequested("FluxNativeQueryExec")
          walk(ensureWholeStageFragmentRoot(unwrapTransparent(bex.child)))

        case reused: ReusedExchangeExec =>
          // Spark reuses the same Exchange JVM object here, but native FLUX cannot
        ...
        case topk: TakeOrderedAndProjectExecTransformer =>
          ...
          // task. In the FLUX dump we model this as a dedicated single-driver
    ...
    // Apply cross-cut Catalyst rules (Sort/Skip-shuffle) here. Plan-C FluxStrategy
    // intercepts queries at planner time and wraps them in FluxNativeQueryExec
    ...
    val singleDriverExchanges =
      ...
      // CudfOrderBy and can OOM before the write starts.
      {
        val embeddedWriteConsumerIds = rangePlanningFragments.iterator
          ...
          .map(_.id)
          .toSet
        // A V2 write keeps its commit node outside FluxNativeQueryExec, so its terminal native
        // fragment does not contain WriteFilesExecTransformer. FluxCollapseRule marks that exact
        ...
        distributedWriteConsumerIds.foreach {
          fragmentId =>
            logInfo(
              s"FluxNativeQueryExec: preserving RANGE for distributed write consumer " +
                s"F$fragmentId (one writer per peer)")
        }
        FluxRangeTopology.collapseRangesForSingleDriverConsumers(
          cappedExchanges,
    ...
    cappedExchanges.zip(singleDriverExchanges).foreach {
      case (before, after) if before.exchangeType == "RANGE" && after.exchangeType == "SINGLE" =>
        logInfo(
          s"FluxNativeQueryExec: planning RANGE exchange ${before.id} " +
    ...
    val sortedExchanges =
      planFluxExchangePartitions(singleDriverExchanges)
        .sortBy(_.id)
        .toSeq
    ...
    fragments.map {
      fragment =>
        if (localInputFragmentIds.contains(fragment.id) && fragment.parallelism != 1) {
          logInfo(
            s"FluxNativeQueryExec: forcing fragment ${fragment.id} to one driver because the " +
  ...
  /**
   * Re-run plan-shape parity rules on the wrapped child plan. FluxStrategy intercepts before the
  ...
  private def applyCrossCutRules(plan: SparkPlan): SparkPlan = {
    val sortRule = FluxSinglePartitionSortRule()
    val skipShuffleRule = FluxRemoveRedundantShuffleRule()
    val parallelSortSplitRule = FluxParallelSortSplitRule()
    val finalAggTopNPartialRule = FluxFinalAggTopNPartialRule()
    val rootTopNPartialRule = FluxRootTopNPartialRule()
    // A V2 writer remains outside FLUX and consumes the wrapped query's root shuffle as its final
    ...
    val (distributedWriteRoot, planToTransform) =
      FluxNativeQueryExec.detachDistributedWriteRoot(plan, distributedWriteOutput)
    ...
    // Plan C wraps the subtree before FluxCollapseRule can normalize Cartesian products. Apply the
    // same replicated-build rewrite here; Plan D is already rewritten and this remains a no-op.
    val afterReplicatedCartesian = FluxReplicatedCartesianRule()(afterNativeUnion)
    val afterSort = sortRule(afterReplicatedCartesian)
    val afterSkipShuffle = skipShuffleRule(afterSort)
    // parallelSortSplit must run AFTER FluxSinglePartitionSortRule so the
    ...
    // Remove Spark's matched Partial and Final WindowGroupLimit pruning operators for cuDF FLUX,
    ...
    // non-cuDF FLUX retains the existing physical plan unless explicitly enabled.
    val rankFilterWindowEnabled = FluxRankFilterWindowRewrite.isEnabled(
      optionalBooleanConf(FluxRankFilterWindowRewrite.EnabledKey),
      GlutenConfig.get.enableColumnarCudf)
    val (afterRankFilterWindow, rankFilterWindowStats) = FluxRankFilterWindowRewrite(
      afterNativeLocalSorts,
      enabled = rankFilterWindowEnabled,
      numPartitions = fluxSparkPartitionCount)
    if (rankFilterWindowStats.rewrittenWindows > 0) {
      logInfo(
        s"FluxNativeQueryExec: selected native Window for " +
    ...
    val (afterWindowInputOrdering, windowInputOrderingStats) =
      FluxWindowInputOrdering(afterRankFilterWindow)
    if (windowInputOrderingStats.candidateWindows > 0) {
      logInfo(
        s"FluxNativeQueryExec: native Window sorted-input verification: candidates=" +
    ...
    val afterSmjHashJoinRewrite = rewriteFluxSortMergeJoinToHashJoin(afterNativeHashJoins)
    val afterBuildSideNormalization = normalizeFluxJoinBuildSide(afterSmjHashJoinRewrite)
    ...
    // FluxNativeQueryExec is injected before the normal columnar post-transform pass can see this
    // subtree. Re-run the flushable partial aggregation rule here so FLUX plans keep the same
    ...
    val reattachedPlan =
      FluxNativeQueryExec.reattachDistributedWriteRoot(distributedWriteRoot, collapsedPlan)
    ...
    FluxBroadcastLifecycle.deferBroadcastPreparation(finalPlan)
    finalPlan
  }
  private def splitWriteRoot(plan: SparkPlan): SparkPlan = {
    ...
    val partitions = positiveIntConf("spark.gluten.mpp.splitWriteRoot.partitions")
      .getOrElse(math.max(1, fluxSparkPartitionCount * 2))
    ...
    if (splitCount > 0) {
      logInfo(
        s"FluxNativeQueryExec: split $splitCount native write root(s) with $partitions HASH " +
  ...
  private def splitPostJoinFinalAgg(plan: SparkPlan): SparkPlan = {
    ...
    if (splitCount > 0) {
      logInfo(
        s"FluxNativeQueryExec: inserted $splitCount post-join final-aggregate split(s) " +
          s"after join hubs")
    }
    rewritten
  }
  ...
  private def splitScalarSubqueryProbeRepartition(plan: SparkPlan): SparkPlan = {
    ...
    if (splitCount > 0) {
      logInfo(
        s"FluxNativeQueryExec: inserted $splitCount scalar-subquery probe repartition split(s)")
    }
    rewritten
  }
    ...
    math.max(
      ...
      SQLConf.get
        .getConfString(
          ...
          fluxSplitHashPartitions().toString)
        .toInt)
  }
  ...
  private def buildPostJoinPartialFinalAgg(
    ...
    if (!validation.ok()) {
      logWarning(
        s"FluxNativeQueryExec: post-join partial aggregate validation failed ($validation); " +
          "leaving aggregate unsplit")
      return None
    }

    val shuffledPartialAgg = ShuffleExchangeExec(
      HashPartitioning(agg.groupingExpressions, fluxPostJoinFinalAggSplitPartitions()),
      partialAgg)
    Some(
      agg.copy(
  ...
  private def fluxPostJoinFinalAggSplitPartitions(): Int = {
    val confKey = "spark.gluten.mpp.postJoinFinalAggSplitPartitions"
    math.max(
      1,
      SQLConf.get
        .getConfString(confKey, fluxSplitHashPartitions().toString)
        .toInt)
  }

  // Unconditional FLUX "EnsureRequirements" for hash-join sides: when the fragment
  ...
  private def splitFinalAggBeforeJoinHub(plan: SparkPlan): SparkPlan = {
    ...
    if (splitCount > 0) {
      logInfo(
        s"FluxNativeQueryExec: inserted $splitCount final-aggregate split(s) before join hubs")
    }
    rewritten
  }
  ...
  private def maybeSplitFinalAggJoinSide(plan: SparkPlan, joinKeys: Seq[Expression]): SparkPlan = {
    ...
    // exchange into a columnar FLUX HASH exchange during
    // ColumnarCollapseTransformStages (GPU-to-GPU; stays FLUX). Trigger is the
    ...
    // for-FLUX reconciling both sides across all distribution requirements is the
    // follow-up.
    if (joinKeys.isEmpty || !isFilteredAggregateJoinSide(plan)) {
      return plan
    }
    ShuffleExchangeExec(HashPartitioning(joinKeys, fluxSplitHashPartitions()), plan)
  }
  ...
  private def splitExistenceFinalBeforeJoinHub(plan: SparkPlan): SparkPlan = {
    ...
    if (splitCount > 0) {
      logInfo(
        s"FluxNativeQueryExec: inserted $splitCount existence-final split(s) before join hubs")
    }
    rewritten
  }
   ...
   *
   * Real FLUX inputs still stay behind the iterator boundary: exchanges, TopN gather slots,
  ...
  private def alignLocalStreamInputs(
      inputs: Seq[FluxLocalStreamInput],
      sparkPartitionCount: Int): Seq[FluxLocalStreamInput] = {
    inputs.map {
      input => input.copy(rdd = FluxNativeQueryRDD.alignInputRDD(input.rdd, sparkPartitionCount))
    }
  }

  private def offloadLocalSorts(plan: SparkPlan): SparkPlan = {
    val withNativeSortPreProjects = plan.transformUp {
      case sort: SortExecTransformer => FluxComputedSortKeyProjection.rewrite(sort)
    }
  ...
  private def offloadLocalHashJoins(plan: SparkPlan): SparkPlan = {
    ...
    def stripHashJoinInputSort(child: SparkPlan): SparkPlan = {
      val stripped = FluxHashJoinInputSortRewrite.strip(child)
    ...
    if (strippedSorts > 0) {
      logInfo(
        s"FluxNativeQueryExec: stripped $strippedSorts local sort(s) " +
          s"from hash join inputs")
    }
    rewritten
  }

  private def rewriteFluxSortMergeJoinToHashJoin(plan: SparkPlan): SparkPlan = {
    if (!rewriteFluxSortMergeJoinToHashJoinEnabled) {
      return plan
    }
    ...
    val rewritten = plan.transformUp {
      case join: SortMergeJoinExecTransformer =>
        fluxSortMergeJoinHashBuildSide(join) match {
          case Some(buildSide) =>
            rewrittenJoins += 1
            logInfo(
              s"FluxNativeQueryExec: rewriting sort-merge join to shuffled hash join " +
                s"for FLUX native execution joinType=${join.joinType} buildSide=$buildSide")
            ShuffledHashJoinExecTransformer(
              join.leftKeys,
              join.rightKeys,
              join.joinType,
    ...
    if (rewrittenJoins > 0) {
      logInfo(
        s"FluxNativeQueryExec: rewrote $rewrittenJoins sort-merge join(s) to shuffled hash join " +
          s"and stripped $strippedSorts local sort(s)")
    }
    rewritten
  }

  private def fluxSortMergeJoinHashBuildSide(
  ...
  private def overloadedFragmentFallbackReason(
    ...
    val maxInbound = fluxNonNegativeIntConf("spark.gluten.mpp.maxInboundExchangesPerFragment", 0)
    val maxHashInbound =
      fluxNonNegativeIntConf("spark.gluten.mpp.maxHashInboundExchangesPerFragment", 0)
    val maxBroadcastInbound =
      fluxNonNegativeIntConf("spark.gluten.mpp.maxBroadcastInboundExchangesPerFragment", 0)
    if (maxInbound <= 0 && maxHashInbound <= 0 && maxBroadcastInbound <= 0) {
      return None
    }
    logInfo(
      s"FluxNativeQueryExec: inbound exchange guardrails enabled " +
  ...
  private def nativeFluxFallbackReason(
      fragments: Seq[NativeFragment],
      exchanges: Seq[ExchangeSpec]): Option[String] = {
  ...
  private def rootOutputMismatchFallbackReason(
    ...
    if (fragments.isEmpty) {
      return Some("no native FLUX fragments were extracted")
    }
  ...
  private def validateFluxStreamInputs(
      fragmentPlans: Seq[Array[Byte]],
      fragments: Seq[NativeFragment],
      exchanges: Seq[ExchangeSpec],
      fusedBroadcastsByConsumer: Map[Int, Seq[FusedBroadcast]],
      localStreamInputs: Seq[FluxLocalStreamInput]): Option[String] = {
        ...
        if (slots.nonEmpty) {
          ...
          if (requiredInputCount > preparedInputCount) {
            ...
            val rootTree = Option(fragment.rootOperator)
              ...
              .getOrElse("<null>")
            logWarning(
              s"FluxNativeQueryExec: fragment ${fragment.id} has unresolved iterator inputs. " +
            ...
            return Some(
              s"fragment ${fragment.id} Substrait has $slotKind " +
                ...
                s"$preparedInputCount FLUX stream input(s): $inboundExchangeCount inbound " +
   ...
   * output shape, usually from duplicated lineitem branches. Native all-stages-up FLUX runs those
  ...
  /**
   * Align FLUX hash-join fragmenting with Presto's replicated join shape. When one side is small
  ...
  private def pushBroadcastJoinIntoProbeExchange(plan: SparkPlan): SparkPlan = {
    ...
    logInfo(
      s"FluxNativeQueryExec: broadcast-into-probe-exchange candidates=" +
    ...
    if (broadcastCandidates.isEmpty) {
      ...
      logInfo(
        s"FluxNativeQueryExec: no transformed broadcast hash join candidate; " +
    ...
    if (pushCount > 0) {
      logInfo(
        s"FluxNativeQueryExec: pushed $pushCount replicated broadcast join(s) into " +
  ...
  private def pushBroadcastJoinIntoProbeExchange(
    ...
    if (!allJoinRefs.forall(allowedRefs.contains)) {
      logDebug(
        s"FluxNativeQueryExec: not pushing broadcast join into probe exchange; " +
  ...
  private def canPushBroadcastJoinIntoProbeExchange(
    ...
    broadcastJoinPushdownMaxBuildBytes match {
      case Some(maxBuildBytes) =>
        ...
        logInfo(
          s"FluxNativeQueryExec: broadcast join is eligible for probe-exchange push; " +
            s"join=${join.nodeName}#${join.id} buildSide=${join.buildSide} " +
  ...
  private def pushBroadcastJoinBelowProbeExchange(
    ...
    plan match {
      ...
      // `hash_partition_key` expression. FLUX recomputes that hash in the native partition
      ...
      case exchange: ShuffleExchangeLike
        ...
        val pushedJoin =
          BroadcastHashJoinExecTransformer(
            ...
            FluxReplicatedJoinBuildInput(build),
        ...
        logInfo(
          s"FluxNativeQueryExec: pushing broadcast join into HASH exchange producer " +
      ...
      case join: ShuffledHashJoinExecTransformer if join.joinType.isInstanceOf[InnerLike] =>
        if (referencesWithin(streamedRefs, join.left.outputSet)) {
          ...
          logInfo(
            s"FluxNativeQueryExec: probe-exchange push path cannot select one side of " +
      ...
      case other =>
        logInfo(
          s"FluxNativeQueryExec: probe-exchange push path stopped at " +
  ...
  private def broadcastJoinPushdownMaxBuildBytes: Option[BigInt] = {
    ...
    if (configured.isEmpty || configured.equalsIgnoreCase("auto")) {
      ...
      parseBytes(configured).orElse {
        logWarning(
          s"FluxNativeQueryExec: invalid byte size for $key=$configured; " +
  ...
  private def coLocateJoinProbeSide[T <: HashJoinLikeExecTransformer](
    ...
    val inlinedStreamed = inlineBoundaryExchange(join.streamedPlan, streamedExchange) match {
      ...
      case None =>
        logWarning(
          s"FluxNativeQueryExec: not co-locating hash join probe side; " +
    ...
    logInfo(
      s"FluxNativeQueryExec: co-locating hash join probe side " +
    ...
    val relocated = buildSide match {
      case BuildLeft =>
        join.withNewChildren(Array(FluxReplicatedJoinBuildInput(join.left), inlinedStreamed))
      case BuildRight =>
        join.withNewChildren(Array(inlinedStreamed, FluxReplicatedJoinBuildInput(join.right)))
    }
  ...
  private def shouldCoLocateBroadcastJoinProbe(
    ...
    join.joinType match {
      ...
      case _ =>
        logDebug(
          s"FluxNativeQueryExec: not co-locating broadcast hash join probe side; " +
    ...
    if (useAutoGuard) {
      if (join.buildSide == BuildLeft) {
        logDebug(
          s"FluxNativeQueryExec: not auto co-locating broadcast hash join probe side; " +
  ...
  private def shouldCoLocateReplicatedJoinProbe(join: ShuffledHashJoinExecTransformer): Boolean = {
    join.joinType match {
      ...
      case _ =>
        logDebug(
          s"FluxNativeQueryExec: not co-locating replicated hash join probe side; " +
            s"join=${join.nodeName} joinType=${join.joinType} is not inner-like")
        return false
    }
    ...
    if (buildExchange.get.isInstanceOf[BroadcastExchangeLike]) {
      logInfo(
        s"FluxNativeQueryExec: co-locating replicated hash join probe side; " +
          s"join=${join.nodeName} buildSide=${join.buildSide} " +
    ...
    inlineExchangeProducer(exchange).flatMap {
      inlinedProducer =>
        ...
        if (replacements == 1) {
          ...
          logWarning(
            s"FluxNativeQueryExec: expected exactly one boundary for exchange ${exchange.id} " +
   ...
   */
  private[execution] def normalizeFluxJoinBuildSide(plan: SparkPlan): SparkPlan = {
    if (!normalizeFluxJoinBuildSideEnabled) {
      return plan
    }
    plan.transformUp {
      ...
      case join: SortMergeJoinExecTransformer
        ...
        logInfo(
          s"FluxNativeQueryExec: rewriting LEFT SEMI sort-merge join to " +
        ...
        ShuffledHashJoinExecTransformer(
          ...
          join.right,
          join.isSkewJoin)
      case join: ShuffledHashJoinExecTransformer if canNormalizeFluxBuildSide(join) =>
        preferredFluxBuildSide(join)
          .filter(_.side != join.buildSide)
          .map {
            choice =>
              logInfo(
                s"FluxNativeQueryExec: normalizing FLUX hash join build side " +
  ...
  private def canNormalizeFluxBuildSide(join: ShuffledHashJoinExecTransformer): Boolean = {
    join.joinType match {
      case _: InnerLike => true
      case LeftSemi => true
      case _: ExistenceJoin if isCandidateFirstExistenceJoin(join) => true
      case LeftOuter | RightOuter
          if normalizeFluxOuterJoinBuildSideEnabled ||
            forceFluxOuterJoinPreservedBuildSideEnabled =>
        true
      case _ => false
    }
  }
   ...
   * side here would silently broaden this FLUX-only optimization.
   */
  ...
  private def rewriteFactBroadcastJoin(
    ...
    // smaller side -- e.g. the upstream FluxFactProbeBroadcastHint correctly
  ...
  // EnsureRequirements (the FLUX fragment walk reads the existing exchange nodes).
  private def flipBroadcastToSmallSide(
    ...
    logInfo(
      s"FluxNativeQueryExec: flipping fact-table BROADCAST hash join build side to the small " +
  ...
  // columnar FLUX HASH exchange during ColumnarCollapseTransformStages, the same proven
  // path as splitFinalAggBeforeJoinHub), and build the smaller side.
  private def demoteToShuffleHashJoin(
      join: BroadcastHashJoinExecTransformer,
      leftBytes: BigInt,
      rightBytes: BigInt): SparkPlan = {
    val numPartitions = fluxSplitHashPartitions()
    ...
    logInfo(
      s"FluxNativeQueryExec: demoting fact x fact BROADCAST hash join to shuffle hash join " +
  ...
  private def preferredFluxBuildSide(
    ...
    if (forceFluxOuterJoinPreservedBuildSideEnabled) {
  ...
  private def preferredLeftSemiBuildSide(
    ...
    // Velox/cuDF also supports the swapped shape via RIGHT_SEMI, so FLUX can build the left side
  ...
  private def isSimpleScanStatsPipeline(plan: SparkPlan): Boolean = plan match {
    case _: ReusedExchangeExec | _: BroadcastQueryStageExec | _: ShuffleQueryStageExec |
        _: InMemoryTableScanExec | _: FluxPreparedChildExec |
        _: org.apache.gluten.extension.FluxSchemaOnlyExec =>
      false
    ...
    val reports = plan.collect {
      case scan: BatchScanExecTransformerBase =>
        try {
          ...
          case NonFatal(e) =>
            logDebug(
              s"FluxNativeQueryExec: source row statistics unavailable for ${scan.nodeName}",
              e)
            None
        }
  ...
  private def normalizeFluxJoinBuildSideEnabled: Boolean = {
    booleanConf("spark.gluten.mpp.normalizeJoinBuildSide", defaultValue = true)
  }

  private def normalizeFluxOuterJoinBuildSideEnabled: Boolean = {
    booleanConf("spark.gluten.mpp.normalizeOuterJoinBuildSide", defaultValue = true)
  }

  private def forceFluxOuterJoinPreservedBuildSideEnabled: Boolean = {
  ...
  private def coLocateReplicatedJoinProbeEnabled: Boolean = {
    ...
    // distribution in native FLUX: scan splits are assigned independently to
    // each peer.  FluxPartitioningPreservingWrapper only carries Catalyst
  ...
  private def rewriteFluxSortMergeJoinToHashJoinEnabled: Boolean = {
    booleanConf(
  ...
  private def smallFluxRangeMaxRows: BigInt = {
  ...
  /**
   * Translate Catalyst shuffle partitioning into FLUX exchange partitioning.
   *
   * `spark.sql.shuffle.partitions` describes Spark shuffle tasks; it is not the native FLUX
   * topology. Native HASH and RANGE exchanges fan out to the FLUX peers instead. A small globally
   ...
   */
  private def planFluxExchangePartitions(exchanges: Seq[ExchangeSpec]): Seq[ExchangeSpec] = {
    val peerCount = math.max(1, fluxSparkPartitionCount)
    exchanges.map {
      case spec if spec.exchangeType == "RANGE" =>
        smallFluxRangeReason(spec) match {
          case Some(reason) =>
            logInfo(
              s"FluxNativeQueryExec: planning small RANGE exchange ${spec.id} " +
          ...
          case None =>
            ...
            if (spec.numPartitions == rangePartitions) {
              ...
              logInfo(
                s"FluxNativeQueryExec: aligning RANGE exchange ${spec.id} " +
                  s"F${spec.producerFragmentId}->F${spec.consumerFragmentId} partitions " +
                  s"from ${spec.numPartitions} to FLUX cap $rangePartitions " +
  ...
  private def smallFluxRangeReason(spec: ExchangeSpec): Option[String] = {
    ...
    planLogicalStats(spec.rangeSamplePlan).flatMap {
      stats =>
        val bytesSmall = isConfidentSize(stats.sizeInBytes) &&
          stats.sizeInBytes <= smallFluxRangeMaxBytes
        val rowsSmall = stats.rowCount.exists(rows => rows >= 0 && rows <= smallFluxRangeMaxRows)
        if (bytesSmall || rowsSmall) {
          Some(
            s"estimatedBytes=${stats.sizeInBytes}, " +
              s"estimatedRows=${stats.rowCount.getOrElse("unknown")}, " +
              s"maxBytes=$smallFluxRangeMaxBytes, maxRows=$smallFluxRangeMaxRows")
        } else {
          None
        }
    }
   ...
   * actual bounds later inside its `RangePartitioner`. FLUX removes that Spark shuffle, so the
   ...
   * end-to-end fully-FLUX execution.
   */
  private def prepareHybridFluxRangeExchanges(
      exchanges: Seq[ExchangeSpec],
      rangeBoundsCache: FluxRangeBoundsGenerator.QueryCache): Seq[ExchangeSpec] = {
    exchanges.map {
      case spec if spec.exchangeType == "RANGE" && spec.rangeBoundsJson.isDefined =>
        require(
          spec.rangeEffectivePartitions.exists(_ > 0),
          s"FLUX RANGE exchange ${spec.id} has serialized bounds but no effective partition count")
        require(
          spec.rangeEffectivePartitions.get <= spec.numPartitions,
          s"FLUX RANGE exchange ${spec.id} effective partitions " +
      ...
      case spec if spec.exchangeType == "RANGE" =>
        require(
          ...
          s"FLUX RANGE exchange ${spec.id} is missing SortOrder metadata")
        require(
          spec.partitionKeys.size == spec.rangeOrdering.size,
          s"FLUX RANGE exchange ${spec.id} lost sort keys: " +
            s"attributes=${spec.partitionKeys.size}, " +
            s"ordering=${spec.rangeOrdering.size}"
        )
        require(
          spec.rangeSamplePlan != null,
          s"FLUX RANGE exchange ${spec.id} is missing its producer sampling plan")
        ...
        // after the FLUX-specific rewrites. Spark rejects that tree with a column-support mismatch
        ...
        if (inferredBounds.isEmpty && representativeScan.isEmpty) {
          logWarning(
            s"FluxNativeQueryExec: *** SPARK PRE-ACTION FOR FLUX RANGE *** exchange=${spec.id} " +
              s"F${spec.producerFragmentId}->F${spec.consumerFragmentId} requestedPartitions=" +
              s"${spec.numPartitions}. FluxRangeBoundsGenerator will execute the Spark producer " +
              s"plan to collect bounded samples before FluxNativeQueryRDD starts; this launch is " +
              s"hybrid preparation and is not an end-to-end fully-FLUX execution.")
        }
        ...
        require(
          ...
          s"FLUX RANGE exchange ${spec.id} computed ${bounds.effectivePartitions} effective " +
            s"partitions for ${spec.numPartitions} requested partitions"
        )
        logInfo(s"FluxNativeQueryExec: RANGE exchange ${spec.id} " +
  ...
  private def inferIntegralRangeBounds(
      plan: SparkPlan,
      ordering: Seq[SortOrder],
      requestedPartitions: Int): Option[FluxRangeBoundsGenerator.Result] = {
    ...
    if (ordering.isEmpty || keyAttribute.isEmpty) {
      logInfo(
        s"FluxNativeQueryExec: RANGE interval inference requires an attribute leading key; " +
    ...
    if (outputKey.isEmpty) {
      logInfo(
        s"FluxNativeQueryExec: RANGE interval inference could not anchor " +
    ...
    val inferred = for {
      ...
      result <- FluxRangeBoundsGenerator.fromIntegralInterval(
        ordering,
        lower,
        upper,
        requestedPartitions,
      ...
      logInfo(
        s"FluxNativeQueryExec: inferred RANGE interval [$lower,$upper) for " +
    ...
    if (inferred.isEmpty) {
      ...
      logInfo(
        s"FluxNativeQueryExec: RANGE interval inference did not apply for " +
  ...
  private def findRangeRepresentativeScan(
    ...
    candidates.sortBy(_._3).headOption.map {
      case (scan, scanOrdering, estimatedBytes) =>
        ...
        logInfo(
          s"FluxNativeQueryExec: using representative RANGE leaf scan " +
  ...
  private def inferParquetFooterRangeBounds(
    ...
    if (maxFiles == 0 || filePaths.size > maxFiles) {
      logInfo(
        s"FluxNativeQueryExec: skipping Parquet footer RANGE inference for ${key.get.name}; " +
    ...
    for {
      ...
      logInfo(
        s"FluxNativeQueryExec: inferred Parquet footer RANGE interval " +
      ...
      fragment =>
        tunedConsumers.get(fragment.id) match {
          case Some(tunedParallelism) if tunedParallelism > fragment.parallelism =>
            logInfo(
              s"FluxNativeQueryExec: raising post-join final-aggregate fragment " +
  ...
  private def tuneMultiInputJoinConsumerParallelism(
    ...
    val requested = positiveIntConf("spark.gluten.mpp.joinDriversPerFragment")
      ...
      .getOrElse(2)
    val inboundCounts = exchanges.groupBy(_.consumerFragmentId).map {
      case (fragmentId, fragmentExchanges) => fragmentId -> fragmentExchanges.size
    }
    fragments.map {
      fragment =>
  ...
  private def fluxSparkPartitionCount: Int = {
    if (!booleanConf("spark.gluten.mpp.multiExecutor.enabled", defaultValue = false)) {
      return 1
    }
    ...
    logDebug(s"FluxNativeQueryExec: multi-executor FLUX requested; target Spark partitions=$count")
    count
  }

  private def effectiveFluxSparkPartitionCount(
      requestedCount: Int,
      exchanges: Seq[ExchangeSpec],
    ...
    if (
      ...
      logDebug(
        s"FluxNativeQueryExec: keeping this FLUX subquery single-peer despite requested " +
    ...
    if (!isWriteJob && Thread.currentThread().getName.startsWith("subquery-")) {
      ...
      if (rootFedBySingleGather) {
        logDebug(
          s"FluxNativeQueryExec: keeping this FLUX query single-peer despite requested " +
  ...
  private def resolveFluxPeers(requestedCount: Int): GlutenFluxPeerResolution = {
    val manualPeerEndpointsJson = stringConf(FLUX_PEER_ENDPOINTS_KEY, "")
    if (
      requestedCount <= 1 ||
      !booleanConf("spark.gluten.mpp.multiExecutor.enabled", defaultValue = false)
    ) {
      return GlutenFluxPeerResolution(Array.empty, manualPeerEndpointsJson)
    }
    GlutenFluxPeerResolver.resolve(sparkContext.getConf, requestedCount, manualPeerEndpointsJson)
  }
    ...
    def collect(plan: SparkPlan): Unit = {
      plan match {
        ...
        case join: HashJoinLikeExecTransformer =>
          ...
          // differ from Spark's left/right child order when the build side is switched. Keep FLUX
  ...
  private def unwrapToExchange(plan: SparkPlan): Option[Exchange] = {
    plan match {
      ...
      case marker: FluxReplicatedJoinBuildInput => boundaryExchange(marker.child)
  ...
  private def isReplicatedJoinBuildInput(plan: SparkPlan): Boolean = {
    plan match {
      case _: FluxReplicatedJoinBuildInput => true
  ...
  /**
   * Materialize only subqueries that still appear in executable expressions after the FLUX scalar
  ...
  private def materializeScalarSubqueries(plan: SparkPlan): Unit = {
    ...
    // eagerly starts regular BroadcastExchange jobs, but FLUX consumes those joins
    // through native BROADCAST exchanges. Preparing the whole plan can therefore
    // driver-collect a large build side before FLUX starts, tripping
    ...
    // outer FLUX graph and can deadlock the 2-GPU task slots.
          ...
          expr =>
            expr.foreach {
              case sub: ExecSubqueryExpression =>
                ...
                logInfo(
                  s"FluxNativeQueryExec: materializing residual " +
    ...
    if (materializedCount == 0) {
      logDebug(
        "materializeScalarSubqueries: no remaining executable subquery expressions; " +
          "rewritten scalars will be supplied by native FLUX exchanges")
    } else if (materializedExecSubqueryCount > 0) {
      logInfo(
        s"FluxNativeQueryExec: materialized $materializedExecSubqueryCount residual " +
          "ExecSubqueryExpression node(s) after native scalar rewrite")
    }
  }

  /**
   * Irreversibly suppress Spark-side broadcast collection for a plan committed to native FLUX.
   *
   * This method must only be called after every branch that can delegate to BSP has completed. Keep
   * the diagnostic switch semantics formerly enforced by FluxSuppressDeadBroadcastsRule: when
   * disabled, native FLUX retains the legacy driver-side collect path for comparison.
   */
  private[gluten] def suppressDeadBroadcastsForNativeFlux(plan: SparkPlan): Unit = {
    val enabled = SQLConf.get
      .getConfString("spark.gluten.mpp.suppressDeadBroadcast", "true")
      .toBoolean
    if (!enabled) {
      logDebug(
        "FluxNativeQueryExec: dead-broadcast suppression disabled by " +
    ...
    def mark(plan: SparkPlan): Unit = {
      ...
      plan match {
        case prepared: FluxPreparedChildExec =>
        ...
        case broadcast: ColumnarBroadcastExchangeExec =>
          if (broadcast.suppressForFluxNativeExecution()) {
            markedCount += 1
          }
      ...
      // the next iteration even though FLUX has already inlined the same work.
    ...
    if (markedCount > 0) {
      logDebug(
        s"FluxNativeQueryExec: suppressed $markedCount ColumnarBroadcastExchangeExec " +
          "node(s) under native FLUX execution")
    }
  }
  ...
  private def classifyPartitioning(partitioning: Partitioning): (String, Seq[Attribute]) = {
    partitioning match {
      ...
      case other =>
        logWarning(
          s"FluxNativeQueryExec: unexpected partitioning type: " +
    ...
    positiveIntConf("spark.gluten.mpp.maxDriversPerFragment") match {
      case Some(maxDrivers) =>
        ...
        if (capped != coreCapped) {
          logDebug(
            s"FluxNativeQueryExec: capping fragment drivers from $coreCapped to $capped " +
  ...
  // parquet chunk/pass read limits for FLUX scans. See FluxNativeQueryRDD companion.
  ...
  private def bytesConf(key: String, defaultValue: BigInt): BigInt = {
    ...
    parseBytes(raw).getOrElse {
      logWarning(s"FluxNativeQueryExec: invalid byte size for $key=$raw; using $defaultValue")
      defaultValue
    }
  }
  ...
  private def fluxNonNegativeIntConf(key: String, defaultValue: Int): Int = {
    ...
    try {
      ...
      if (parsed < 0) {
        logWarning(
          s"FluxNativeQueryExec: invalid negative integer for $key=$raw; using $defaultValue")
        defaultValue
      } else {
        parsed
      }
      ...
      case _: NumberFormatException =>
        logWarning(s"FluxNativeQueryExec: invalid integer for $key=$raw; using $defaultValue")
        defaultValue
    }
  }
   ...
   * which is what we want for each FLUX fragment.
   *
   * @return
  ...
  private def generateSubstraitForFragment(fragment: NativeFragment): Array[Byte] = {
    fragment.rootOperator match {
      ...
      case other =>
        throw new IllegalStateException(
          s"FluxNativeQueryExec: fragment ${fragment.id} root operator " +
  ...
  private def generateSubstraitPlan(fragment: NativeFragment): Array[Byte] = {
    ...
    rootOp match {
      case ts: TransformSupport =>
        ...
        logDebug(
          s"FluxNativeQueryExec: fragment ${fragment.id} Substrait plan: " +
      ...
      spec =>
        ...
        val indices = spec.partitionKeys.flatMap {
          attr =>
            ...
            if (idx < 0) {
              logWarning(
                s"FluxNativeQueryExec: partition key ${attr.name}#${attr.exprId.id} not in " +
        ...
        if (spec.exchangeType == "RANGE" && indices.size != spec.partitionKeys.size) {
          throw new IllegalStateException(
            s"FLUX RANGE exchange ${spec.id} cannot resolve every sort key against producer " +
        ...
        val rangeFields =
          if (spec.exchangeType == "RANGE") {
            val bounds = spec.rangeBoundsJson.getOrElse {
              throw new IllegalStateException(
                s"FLUX RANGE exchange ${spec.id} has no serialized Spark boundaries")
            }
            val effective = spec.rangeEffectivePartitions.getOrElse {
              throw new IllegalStateException(
                s"FLUX RANGE exchange ${spec.id} has no effective partition count")
        ...
        s"""{
           ...
           |  "exchangeNodeId": "flux_exchange_source_${spec.id}",
  ...
  // --- Scan split info extraction for FLUX Phase 3 ---
   ...
   * This gives the native FluxQueryCoordinator all file paths to scan.
   *
   ...
   *
   * In BSP, each partition gets its own SplitInfo (a subset of files). For FLUX, the coordinator
    ...
    splitInfos.foreach {
      si =>
        val localFiles = si match {
          ...
          case other =>
            throw new IllegalStateException(
              s"FluxNativeQueryExec: unsupported FLUX split info " +
   ...
   * (`flux-substrait-runner`) replay the plan directly against Velox-cudf without going through
   * Spark.
   *
  ...
  private def dumpPlanIfEnabled(
    ...
    try {
      ...
      // the `exchangeSpecsJson` parameter to nativeCreateFluxQuery. The C++ harness can feed this
      ...
      logInfo(
        s"FluxNativeQueryExec: dumped Substrait plan " +
          s"(${fragmentPlans.length} fragments, ${exchangeSpecs.size} exchanges, " +
          s"$totalSplitFiles split files) to $outDir")
    } catch {
      case t: Throwable =>
        logWarning(
          s"FluxNativeQueryExec: failed to dump Substrait plan to '$dumpDir' " +
  ...
  private def postFluxPlanEvent(
    ...
    if (executionId.isEmpty) {
      logDebug("FluxNativeQueryExec: skip FLUX plan event because SQL execution id is unavailable")
      return
    }
    ...
    val maxChars = fluxNonNegativeIntConf("spark.gluten.mpp.veloxPlan.eventLog.maxChars", 262144)
    val (plans, captureError) =
      if (!requestedCaptureEnabled) {
        ...
        (Seq.empty[String], "Final Velox plan capture skipped for single-fragment FLUX plan.")
  ...
  private def captureFinalVeloxPlans(
    ...
    try {
      val runtime = Runtimes.contextInstance(BackendsApiManager.getBackendName, "FluxQueryExplain")
      val jniWrapper = FluxQueryJniWrapper.create(runtime)
      val plans = jniWrapper.nativeExplainFluxQuery(
        fragmentPlans,
        numDriversPerFragment,
        exchangeSpecsJson.getBytes(StandardCharsets.UTF_8),
        fragmentSplitInfos,
      ...
      case t: Throwable =>
        logWarning(
          s"FluxNativeQueryExec: failed to capture final Velox plan for event log " +
            s"(${t.getClass.getSimpleName}: ${t.getMessage}); continuing with native execution",
          t
        )
  ...
  private def truncatePlanFragments(
      plans: Seq[String],
      maxChars: Int): Seq[GlutenFluxPlanFragmentEvent] = {
