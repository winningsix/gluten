 ...
 * between two FLUX fragments.
 *
 * In FLUX mode, each fragment that feeds into a downstream fragment via a streaming GPU exchange
 * needs a sink operator that partitions and buffers the output for consumption by the downstream
 * fragment's [[FluxExchangeSourceTransformer]].
 *
 ...
 */
case class FluxExchangeSinkTransformer(
    child: SparkPlan,
    exchangeId: Int,
    numPartitions: Int,
  ...
  override def nodeName: String =
    s"FluxExchangeSink(exchange=$exchangeId, partitions=$numPartitions)"

  override def simpleString(maxFields: Int): String = {
    val keysStr = if (partitionKeys.nonEmpty) {
      s", keys=${partitionKeys.map(_.name).mkString("[", ", ", "]")}"
    } else ""
    s"FluxExchangeSink(exchange=$exchangeId, partitions=$numPartitions$keysStr)"
  }
  ...
  override protected def doTransform(context: SubstraitContext): TransformContext = {
    ...
    // FLUX plan compilation, where it wraps the fragment's plan root with
