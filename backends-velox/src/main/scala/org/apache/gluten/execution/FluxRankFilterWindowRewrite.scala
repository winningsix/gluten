 ...
 * the upper Filter.
 *
 * A partitioned Window must see every row for a partition on the same FLUX peer. Spark normally
 ...
 */
private[execution] object FluxRankFilterWindowRewrite {
  ...
  private def findRequiredLocalSort(window: WindowExecTransformer): Option[LocalSortPath] = {
    def find(
      ...
      case sort: SortExecTransformer if !sort.global =>
        if (FluxWindowInputOrdering.hasRequiredOrdering(sort, partitionSpec, orderSpec)) {
