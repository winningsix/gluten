...
class FluxColumnarTransitionBridgeSuite extends SharedSparkSession {
  ...
  test("bridges Gluten R2C over Spark C2R without executing either transition") {
    ...
    assert(FluxColumnarTransitionBridge()(plan) eq child)
  }
  ...
  test("bridges same-family transition pairs") {
    ...
    assert(FluxColumnarTransitionBridge()(RowToColumnarExec(ColumnarToRowExec(child))) eq child)
    assert(
      FluxColumnarTransitionBridge()(RowToVeloxColumnarExec(VeloxColumnarToRowExec(child))) eq
        child)
  }

  test("bridges an identity C2R below an FLUX-absorbed columnar shuffle") {
    val child = leaf()
    ...
    val rewritten = FluxColumnarTransitionBridge()(exchange)
  ...
  test("does not bridge across a real row operator") {
    ...
    val rewritten = FluxColumnarTransitionBridge()(plan)
  ...
  test("does not treat an isolated transition as a columnar bridge") {
    ...
    assert(FluxColumnarTransitionBridge()(c2r).isInstanceOf[ColumnarToRowExec])
    assert(FluxColumnarTransitionBridge()(r2c).isInstanceOf[RowToColumnarExec])
  }
}
