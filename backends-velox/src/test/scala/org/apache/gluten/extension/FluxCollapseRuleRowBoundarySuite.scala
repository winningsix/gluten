...
class FluxCollapseRuleRowBoundarySuite extends SparkFunSuite {
  ...
  test("strict FLUX elides query HASH directly below an Iceberg RANGE exchange") {
    val child = nativeLeaf()
    ...
    val rule = FluxCollapseRule(new GlutenConfig(SQLConf.get))

    assert(rule.isFullyNativeSupported(range))
    val collapsed = rule(range).asInstanceOf[FluxNativeQueryExec]
  ...
  test("distributed V2 write preserves its root shuffle during dynamic extraction preparation") {
    ...
    val normalized = rule.normalizeFluxNativeOperators(writerDistribution)
    val (rootExchange, producer) =
      FluxNativeQueryExec.detachDistributedWriteRoot(normalized, distributedWriteOutput = true)
    val prepared = FluxNativeQueryExec.reattachDistributedWriteRoot(rootExchange, producer)
  ...
  test("V2 write normalization strips Spark C2R directly over Gluten ColumnarExchange") {
    ...
    val result = FluxCollapseRule.normalizeV2WriteExchangeAdapters(adapter)
  ...
  test("V2 write normalization finds only exact exchange adapters inside a larger query") {
    ...
    val result = FluxCollapseRule.normalizeV2WriteExchangeAdapters(parent)
  ...
  test("terminal root Spark C2R is removed before FLUX row egress") {
    val child = nativeLeaf()
    ...
    assert(collapsed.isInstanceOf[FluxNativeQueryExec])
  ...
  test("Gluten terminal root native-to-row output is also an intentional FLUX egress") {
    val child = nativeLeaf()
    ...
    assert(collapsed.children.head.isInstanceOf[FluxNativeQueryExec])
    assert(!rule.isFullyNativeSupported(boundary))
  }
  ...
  test("root DeserializeToObject without a direct C2R remains rejected") {
    ...
    assert(result.find(_.isInstanceOf[FluxNativeQueryExec]).isEmpty)
    assert(!rule.isFullyNativeSupported(boundary))
  }

  test("root DeserializeToObject removes Spark C2R before FLUX row egress") {
    val child = nativeLeaf()
  ...
  test("root DeserializeToObject preserves its one direct Gluten C2R outside FLUX") {
    val child = nativeLeaf()
    ...
    assert(c2r.children.head.isInstanceOf[FluxNativeQueryExec])
  ...
  test("root DeserializeToObject composes Spark C2R with the exact ExistingRDD ingress hybrid") {
    ...
    assert(collapsed.children.head.isInstanceOf[FluxNativeQueryExec])
    assert(collapsed.find(_.isInstanceOf[ColumnarToRowExec]).isEmpty)
    assert(collapsed.find(node => FluxJvmStreamInputMatcher.scan(node).contains(scan)).isDefined)
  }
  ...
  test("root DeserializeToObject composes Gluten C2R with the exact ExistingRDD ingress hybrid") {
    ...
    assert(c2r.isInstanceOf[VeloxColumnarToRowExec])
    assert(c2r.children.head.isInstanceOf[FluxNativeQueryExec])
    assert(c2r.find(node => FluxJvmStreamInputMatcher.scan(node).contains(scan)).isDefined)
  }
  ...
  test("root DeserializeToObject repairs a sortable row shell over ExistingRDD ingress") {
    ...
    val c2r = collapsed.children.head
    assert(c2r.isInstanceOf[VeloxColumnarToRowExec])
    val flux = c2r.children.head
    assert(flux.isInstanceOf[FluxNativeQueryExec])
    assert(flux.find(_.isInstanceOf[SortExecTransformer]).isDefined)
    assert(flux.find(_.isInstanceOf[VeloxColumnarToRowExec]).isEmpty)
    assert(flux.find(node => FluxJvmStreamInputMatcher.scan(node).contains(scan)).isDefined)
  }

  test("nested DeserializeToObject remains a strict FLUX rejection") {
  ...
  test("terminal root egress connects the exact ExistingRDD ingress hybrid to FLUX") {
    val attr = AttributeReference("a", IntegerType, nullable = true)()
    val scan = existingRddScan(attr)
    val ingress = RowToVeloxColumnarExec(scan)
    val nativeSuffix = ProjectExecTransformer(ingress.output, ingress)
    ...
    assert(collapsed.isInstanceOf[FluxNativeQueryExec])
    val flux = collapsed.asInstanceOf[FluxNativeQueryExec]
    assert(collapsed.find(_.isInstanceOf[ColumnarToRowExec]).isEmpty)
    assert(flux.child.find(node => FluxJvmStreamInputMatcher.scan(node).contains(scan)).isDefined)
  }
  ...
  test("an ingress-only ExistingRDD plan gets an identity native fragment anchor") {
    ...
    assert(collapsed.isInstanceOf[FluxNativeQueryExec])
    val flux = collapsed.asInstanceOf[FluxNativeQueryExec]
    assert(flux.child.isInstanceOf[ProjectExecTransformer])
    assert(flux.child.output.map(_.exprId) == ingress.output.map(_.exprId))
    assert(flux.child.find(node => FluxJvmStreamInputMatcher.scan(node).contains(scan)).isDefined)
  }

  test("a nested native-to-row boundary cannot activate FLUX") {
  ...
  test("ExistingRDD hybrid validation accepts only an exact RDD ingress below a native suffix") {
    ...
    assert(FluxJvmStreamInputMatcher.scan(ingress).contains(scan))
    assert(
      FluxJvmStreamInputMatcher
  ...
  test("ExistingRDD hybrid captures the exact SerializeFromObject ExternalRDD ingress") {
    ...
    assert(!rule.isFullyNativeSupported(nativeSuffix))
    assert(rule.isSupportedJvmStreamHybridPlan(nativeSuffix))
    assert(FluxJvmStreamInputMatcher.rowInput(ingress).contains(serializer))
    assert(
      FluxJvmStreamInputMatcher
    ...
    assert(
      collapsed
        .find(node => FluxJvmStreamInputMatcher.rowInput(node).contains(serializer))
        .isDefined)

    val guardOutcomes = FluxJvmStreamInputMatcher.objectIngressGuardOutcomes(nativeSuffix)
  ...
  test("ExistingRDD object ingress rejects non-direct and non-ExternalRDD serializers") {
    ...
    assert(FluxJvmStreamInputMatcher.rowInput(directExternalIngress).isEmpty)
    assert(
    ...
    Seq(
      ...
      serializer =>
        val ingress = RowToVeloxColumnarExec(serializer)
        assert(FluxJvmStreamInputMatcher.rowInput(ingress).isEmpty)
        assert(
          !rule.isSupportedJvmStreamHybridPlan(ProjectExecTransformer(ingress.output, ingress)))
    }

    val adapterGuards =
      FluxJvmStreamInputMatcher.objectIngressGuardOutcomes(doubleInputAdapterSerializer)
    ...
    val referenceGuards =
      FluxJvmStreamInputMatcher.objectIngressGuardOutcomes(foreignReferencedSerializer)
  ...
  test("ExistingRDD hybrid validation absorbs a shuffle directly over the exact ingress") {
    ...
    assert(collapsed.isInstanceOf[FluxNativeQueryExec])
    val nativePlan = collapsed.asInstanceOf[FluxNativeQueryExec].child
    ...
    assert(nativePlan.find(node => FluxJvmStreamInputMatcher.scan(node).contains(scan)).isDefined)
  }
  ...
  test("ExistingRDD hybrid repairs a computed sort on a native-only sibling branch") {
    ...
    val nativePlan = rule.normalizeFluxNativeOperators(
      union,
  ...
  test("ExistingRDD hybrid normalizes a collect-list row shell across a native union") {
    ...
    val normalized = rule.normalizeFluxNativeOperators(
      stalePlan,
      preserveJvmStreamIngress = true,
      rewriteNativeUnion = _.transformUp {
    ...
    assert(
      nativeAggregate.aggregateExpressions.forall(
        _.aggregateFunction.children.forall(_.isInstanceOf[AttributeReference])))
    assert(nativeAggregate.child.isInstanceOf[ProjectExecTransformer])
    assert(normalized.find(node => FluxJvmStreamInputMatcher.scan(node).contains(scan)).isDefined)
  }
  ...
  test("ExistingRDD matcher rejects non-Existing RDDScan names") {
    ...
    assert(FluxJvmStreamInputMatcher.scan(ingress).isEmpty)
    assert(!rule.isSupportedJvmStreamHybridPlan(nativeSuffix))
  }
  ...
  test("ExistingRDD matcher rejects a native-to-row round trip") {
    ...
    assert(FluxJvmStreamInputMatcher.scan(roundTrip).isEmpty)
    assert(
  ...
  test("strict normalization materializes computed sort keys and removes stale transitions") {
    ...
    val normalized = rule.normalizeFluxNativeOperators(stalePlan)
  ...
  test("strict normalization materializes computed keys on an already-native sort") {
    ...
    val normalized = rule.normalizeFluxNativeOperators(nativeSort)
  ...
  Seq(false, true).foreach {
    flushable =>
      ...
      test(s"strict normalization pre-projects an already native $implementation aggregate") {
        ...
        val normalized = rule.normalizeFluxNativeOperators(original)
  ...
  test("bounded scalar result recognizes only a global aggregate without grouping") {
    ...
    assert(FluxCollapseRule.isBoundedScalarResult(scalarPlan))
    assert(!FluxCollapseRule.isBoundedScalarResult(groupedPlan))
    assert(!FluxCollapseRule.isBoundedScalarResult(nativeLeaf()))
  }
}
