...
class MppReplicatedCartesianRuleSuite extends QueryTest with SharedSparkSession {
  ...
  test("chooseBuildSide selects only a stats-bounded complete input") {
    ...
    assert(MppReplicatedCartesianRule.chooseBuildSide(small, large, max).contains(BuildLeft))
    assert(MppReplicatedCartesianRule.chooseBuildSide(large, small, max).contains(BuildRight))
    assert(MppReplicatedCartesianRule.chooseBuildSide(small, smaller, max).contains(BuildRight))
    assert(MppReplicatedCartesianRule.chooseBuildSide(None, large, max).isEmpty)
    assert(MppReplicatedCartesianRule.chooseBuildSide(large, large, max).isEmpty)
  }
  ...
  test("right small side becomes a native broadcast nested-loop build") {
    withSQLConf(MAX_BUILD_BYTES_KEY -> "1kb", "spark.gluten.mpp.failOnFallback" -> "true") {
      ...
      val rewritten = MppReplicatedCartesianRule()(cartesian(left, right))
  ...
  test("left small side is broadcast and original schema order is restored") {
    withSQLConf(MAX_BUILD_BYTES_KEY -> "1kb", "spark.gluten.mpp.failOnFallback" -> "true") {
      ...
      val rewritten = MppReplicatedCartesianRule()(original)
