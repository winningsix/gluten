...
class FluxSuppressDeadBroadcastsRuleSuite extends QueryTest with SharedSparkSession {
  ...
  private def flux(child: SparkPlan): FluxNativeQueryExec =
    FluxNativeQueryExec(child, fragments = Seq.empty, exchanges = Seq.empty)
  ...
  test("defers eager prepare but keeps RANGE broadcasts live") {
    ...
    FluxSuppressDeadBroadcastsRule()(flux(range(exchange)))

    assert(!exchange.isFluxSuppressed)
    assert(exchange.isFluxPrepareDeferred)
  }

  test("defers a direct broadcast during final planning") {
    val exchange = broadcast("broadcast_key")

    FluxSuppressDeadBroadcastsRule()(flux(exchange))
  ...
  test("runtime commit suppresses a direct broadcast after validation") {
    ...
    flux(exchange).suppressDeadBroadcastsForNativeFlux(exchange)
  ...
  test("keeps a broadcast shared by RANGE and direct branches live") {
    ...
    FluxSuppressDeadBroadcastsRule()(flux(shared))
  ...
  test("keeps a reused broadcast under RANGE live") {
    ...
    FluxSuppressDeadBroadcastsRule()(flux(range(reused)))
  ...
  test("keeps a subquery broadcast under RANGE live") {
    ...
    FluxSuppressDeadBroadcastsRule()(flux(range(subquery)))
  ...
  test("defers a broadcast referenced only by an executable subquery expression") {
    ...
    FluxSuppressDeadBroadcastsRule()(flux(project))
  ...
  test("runtime commit suppresses a broadcast referenced only by a subquery expression") {
    ...
    flux(project).suppressDeadBroadcastsForNativeFlux(project)
