...
class FluxRangeTopologySuite extends AnyFunSuite {
  ...
  test("preserves RANGE for a distributed one-writer-per-peer consumer") {
    ...
    val Seq(unchanged) = FluxRangeTopology.collapseRangesForSingleDriverConsumers(
      Seq(range),
      fragments,
  ...
  test("uses the effective RANGE partition count for native destinations") {
    ...
    val Seq(effectiveRange, unchangedHash) =
      FluxRangeTopology.applyEffectivePartitionCounts(Seq(range, hash))
  ...
  test("rejects missing or invalid effective RANGE partition counts") {
    ...
    intercept[IllegalStateException] {
      FluxRangeTopology.applyEffectivePartitionCounts(Seq(range(4, None)))
    }
    intercept[IllegalArgumentException] {
      FluxRangeTopology.applyEffectivePartitionCounts(Seq(range(4, Some(0))))
    }
    intercept[IllegalArgumentException] {
      FluxRangeTopology.applyEffectivePartitionCounts(Seq(range(4, Some(5))))
    }
  }
}
