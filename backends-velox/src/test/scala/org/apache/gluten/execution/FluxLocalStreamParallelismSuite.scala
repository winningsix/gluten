...
class FluxLocalStreamParallelismSuite extends AnyFunSuite {

  test("JVM local stream restricts only its consumer fragment") {
    val key = AttributeReference("key", LongType, nullable = false)()
    val source = FluxExchangeSourceTransformer(0, Seq(key))
    val exec = FluxNativeQueryExec(source, Seq.empty, Seq.empty)
    val fragments = Seq(
