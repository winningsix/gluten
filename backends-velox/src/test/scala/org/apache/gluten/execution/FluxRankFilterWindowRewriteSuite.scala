...
class FluxRankFilterWindowRewriteSuite extends AnyFunSuite {
  private val SentinelTag = TreeNodeTag[String]("flux.rank-filter-window-rewrite.sentinel")
  ...
  private case class PartialOptions(
      ...
      fluxBoundaryWrappers: Boolean = false,
  ...
  test("rollout follows cuDF by default and accepts an explicit override") {
    assert(!FluxRankFilterWindowRewrite.isEnabled(configured = None, cudfEnabled = false))
    assert(FluxRankFilterWindowRewrite.isEnabled(configured = None, cudfEnabled = true))
    assert(!FluxRankFilterWindowRewrite.isEnabled(configured = Some(false), cudfEnabled = true))
    assert(FluxRankFilterWindowRewrite.isEnabled(configured = Some(true), cudfEnabled = false))
  }

  test("an explicit disable leaves the rank-filter plan unchanged") {
    val plan = rankFilterBranch("disabled", includeExchange = false)
    val (rewritten, stats) = FluxRankFilterWindowRewrite(plan, enabled = false, numPartitions = 4)
  ...
  test("three rank-filter branches preserve Window Filter and local Sort without TopN") {
    ...
    val (rewritten, stats) = FluxRankFilterWindowRewrite(plan, enabled = true, numPartitions = 4)
  ...
  test("FLUX value-stream wrappers and the synthetic hash project are retained") {
    val plan = rankFilterBranch(
      "flux_wrapped",
      includeExchange = true,
      options = BranchOptions(partial = PartialOptions(fluxBoundaryWrappers = true)))
  ...
  test("only the exact synthetic hash project may bridge an exchange to Partial") {
    Seq("wrong_name", "non_hash", "wrong_keys", "wrong_seed", "reordered_tail").foreach {
      hashProjectKind =>
        ...
        val (rewritten, stats) =
          FluxRankFilterWindowRewrite(plan, enabled = true, numPartitions = 4)
  ...
  test("an extra node inside WholeStage keeps both pruning boundaries") {
    val plan = rankFilterBranch(
      ...
      includeExchange = true,
      options = BranchOptions(
        partial = PartialOptions(fluxBoundaryWrappers = true, wholeStageExtraFilter = true)))
  ...
  private def rankFilterBranch(
    ...
    val exchangeInput =
      if (options.partial.fluxBoundaryWrappers) {
