...
class MppCandidateFirstExistenceBuildSideSuite extends QueryTest with SharedSparkSession {
  ...
  private def existenceJoin(existsName: String, buildSide: BuildSide = BuildRight): TestJoin = {
    ...
    val left = MppExchangeSourceTransformer(0, Seq(leftKey))
    val right = MppExchangeSourceTransformer(1, Seq(rightKey))
    TestJoin(
  ...
  private def normalize(join: ShuffledHashJoinExecTransformer): ShuffledHashJoinExecTransformer = {
    MppNativeQueryExec(join, Seq.empty, Seq.empty)
      .normalizeMppJoinBuildSide(join)
