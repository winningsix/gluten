...
class GlutenFluxQueryControlRegistrySuite extends AnyFunSuite {
  private val run = FluxQueryRunId("q", 7, 0)
  ...
  private def peer(
      ...
      executor: String = "",
      session: String = "",
      state: String = FluxPeerState.Running,
      accepted: Long = 0L): FluxPeerSnapshot = {
  ...
  private def heartbeat(
      ...
      failures: Seq[FluxQueryFailure] = Nil,
      terminals: Seq[FluxTerminalEvent] = Nil): FluxQueryHeartbeat =
    FluxQueryHeartbeat(
      snapshot.executorId,
  ...
  private def failure(snapshot: FluxPeerSnapshot, id: String, message: String): FluxQueryFailure =
    FluxQueryFailure(
      id,
      snapshot.runId,
      snapshot.peerIndex,
  ...
  test("run enters RUNNING only after every expected peer joins") {
    var now = 0L
    val registry = new GlutenFluxQueryControlRegistry(5000L, clock = () => now)

    registry.processHeartbeat(heartbeat(peer(0)))
    assert(registry.snapshot(run).get.state == FluxQueryRunState.Starting)

    registry.processHeartbeat(heartbeat(peer(1)))
    val snapshot = registry.snapshot(run).get
    assert(snapshot.state == FluxQueryRunState.Running)
  ...
  test("first peer failure is preserved and abort commands replay until accepted") {
    ...
    val snapshot = registry.snapshot(run).get
    assert(snapshot.state == FluxQueryRunState.Aborting)
  ...
  test("duplicate task attempt aborts both the original and duplicate attempts") {
    val registry = new GlutenFluxQueryControlRegistry(5000L)
  ...
  test("driver authorizes close only after every peer reaches output EOS") {
    ...
    val first = registry.processHeartbeat(heartbeat(p0.copy(state = FluxPeerState.OutputComplete)))
    assert(first.peerCompletions.isEmpty)
    assert(registry.snapshot(run).exists(_.state == FluxQueryRunState.Running))

    val last = registry.processHeartbeat(heartbeat(p1.copy(state = FluxPeerState.OutputComplete)))
    assert(last.peerCompletions.map(_.peerIndex) == Seq(1))
    val completing = registry.snapshot(run).get
    assert(completing.state == FluxQueryRunState.Completing)
    assert(completing.peers.forall(_.outputComplete))
    assert(completing.peers.forall(peer => !peer.terminal))

    val retry = registry.processHeartbeat(heartbeat(p0.copy(state = FluxPeerState.OutputComplete)))
  ...
  test("non-success peer terminal before output EOS aborts the remaining peers") {
    ...
    registry.processHeartbeat(heartbeat(p0.copy(state = FluxPeerState.Aborted)))
    val snapshot = registry.snapshot(run).get
    assert(snapshot.state == FluxQueryRunState.Aborting)
  ...
  test("successful Spark task end is an implicit output EOS") {
    ...
    assert(oneFinished.state == FluxQueryRunState.Running)
    ...
    assert(finished.state == FluxQueryRunState.Terminal)
  ...
  test("terminal tombstone rejects late traffic and expires after retention") {
    var now = 0L
    val registry = new GlutenFluxQueryControlRegistry(
      peerTimeoutMs = 5000L,
    ...
    registry.processHeartbeat(heartbeat(p0))
    registry.processHeartbeat(heartbeat(p1))
    registry.processHeartbeat(heartbeat(p0.copy(state = FluxPeerState.OutputComplete)))
    registry.processHeartbeat(heartbeat(p1.copy(state = FluxPeerState.OutputComplete)))

    def terminal(snapshot: FluxPeerSnapshot): FluxTerminalEvent =
      FluxTerminalEvent(
    ...
    assert(registry.snapshot(run).exists(_.state == FluxQueryRunState.Terminal))
