...
class GlutenFluxEndpointRegistrySuite extends AnyFunSuite {
  ...
  private def rec(
      ...
      state: String = FluxExecutorEndpointRecord.StateLive): FluxExecutorEndpointRecord =
  ...
  test("empty snapshot is Nil and generation starts at zero") {
    val r = GlutenFluxEndpointRegistry()
