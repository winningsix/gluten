...
class FluxRangeBoundsGeneratorSuite extends SparkFunSuite {
  ...
  test("RANGE preparation materializes shuffle dependencies in topological order") {
    ...
    try {
      ...
      val dependencies =
        FluxRangeBoundsGenerator.shuffleDependenciesInTopologicalOrder(secondShuffle)
      assert(dependencies.size == 2)
      assert(dependencies.map(_.shuffleId).distinct.size == 2)

      FluxRangeBoundsGenerator.materializeShuffleDependencies(secondShuffle)
  ...
  test("footer interval inference is limited to integral keys and bounded file sets") {
    assert(FluxRangeBoundsGenerator.supportsIntegralInterval(IntegerType))
    assert(FluxRangeBoundsGenerator.supportsIntegralInterval(DateType))
    assert(!FluxRangeBoundsGenerator.supportsIntegralInterval(StringType))
  }
  ...
  test("integral predicate bounds interpolate encoded dates by calendar day") {
    ...
    val result = FluxRangeBoundsGenerator
    ...
    StringType.getClass.getMethods
      ...
      .foreach {
        applyByCollationName =>
          ...
          assert(!FluxRangeBoundsGenerator.supports(collated))
      }
  }
  ...
  test("one-pass priority reservoir enforces row limits deterministically") {
    ...
    val byRows = FluxRangeBoundsGenerator.sketchPartition(
      rows.iterator,
      maxRows = 7,
      maxSerializedKeyBytes = Long.MaxValue,
      seed = 17L)
    val repeated = FluxRangeBoundsGenerator.sketchPartition(
      rows.iterator,
      maxRows = 7,
      maxSerializedKeyBytes = Long.MaxValue,
      seed = 17L)
  ...
  test("variable-width reservoir matches pure row-priority selection") {
    ...
    val sketch = FluxRangeBoundsGenerator.sketchPartition(
      rows.iterator,
      maxRows = 3,
  ...
  test("byte admission fails only when a selected priority key cannot fit") {
    ...
    val selectedFailure = intercept[IllegalArgumentException] {
      FluxRangeBoundsGenerator.sketchPartition(
        Iterator(large),
        maxRows = 1,
    ...
    val unaffected = FluxRangeBoundsGenerator.sketchPartition(
  ...
  test("weighted skew samples produce monotonic Spark-compatible bounds") {
    ...
    val sketches = Array(
      FluxRangeBoundsGenerator.PartitionSketch(
        count = 800L,
        samples = heavy,
    ...
    val bounds = FluxRangeBoundsGenerator.determineBounds(
      partitions = 4,
      inputPartitions = 2,
  ...
  test("driver validation fails closed on collected row and byte budget overflow") {
    ...
    val overflow = FluxRangeBoundsGenerator.PartitionSketch(
      count = 2L,
      samples = Array(row, row.copy()),
      serializedKeyBytes = rowBytes * 2L)
  ...
  test("sparse partition samples fail closed unless the full input is smaller than RANGE width") {
    ...
    val one = row(1)
    val two = row(2)
    val empty = FluxRangeBoundsGenerator.PartitionSketch(
      count = 0L,
      samples = Array.empty,
    ...
    val sparseSmall = Array(
      ...
      Array.fill(15)(empty)
    val smallBounds = FluxRangeBoundsGenerator.determineBounds(
      partitions = 8,
  ...
  test("query cache computes equivalent RANGE bounds once under concurrent access") {
    ...
    val ordering = Seq(SortOrder(key, Ascending, NullsFirst, Seq.empty))
    val cache = new FluxRangeBoundsGenerator.QueryCache("execution-1")
    ...
    try {
      val first = pool.submit(new Callable[(FluxRangeBoundsGenerator.Result, Boolean)] {
        override def call(): (FluxRangeBoundsGenerator.Result, Boolean) = {
          cache.getOrCompute(plan, plan.output, ordering, requestedPartitions = 8) {
            ...
            FluxRangeBoundsGenerator.Result("first", boundaryCount = 7)
          }
        }
      })
      assert(ownerStarted.await(10, TimeUnit.SECONDS))
      val second = pool.submit(new Callable[(FluxRangeBoundsGenerator.Result, Boolean)] {
        override def call(): (FluxRangeBoundsGenerator.Result, Boolean) = {
          cache.getOrCompute(plan, plan.output, ordering, requestedPartitions = 8) {
            calls.incrementAndGet()
            FluxRangeBoundsGenerator.Result("unexpected", boundaryCount = 0)
          }
        }
      })
  ...
  test("query cache separates ordering and partition count and evicts failures") {
    ...
    val cache = new FluxRangeBoundsGenerator.QueryCache("execution-2")
    val calls = new AtomicInteger(0)

    def result(label: String): FluxRangeBoundsGenerator.Result = {
      calls.incrementAndGet()
      FluxRangeBoundsGenerator.Result(label, boundaryCount = 1)
    }
    ...
    val failed = new FluxRangeBoundsGenerator.QueryCache("execution-3")
  ...
  test("concurrent cache failure reaches all waiters before a clean retry generation") {
    ...
    val plan = LocalTableScanExecCompat(Seq(key), Seq.empty[InternalRow])
    val ordering = Seq(SortOrder(key, Ascending, NullsFirst, Seq.empty))
    val cache = new FluxRangeBoundsGenerator.QueryCache("execution-failure")
    ...
    try {
      ...
      val retry = cache.getOrCompute(plan, plan.output, ordering, requestedPartitions = 8) {
        calls.incrementAndGet()
        FluxRangeBoundsGenerator.Result("retry", boundaryCount = 7)
      }
