...
// Load a Gluten-dumped Substrait plan directory into MPP fragment + exchange
// specs suitable for MppQueryCoordinator. Mirrors the fragment-building stage
// of MppJniWrapper::nativeCreateMppQuery, but reads from disk instead of
// JNI arrays, so that mpp-substrait-runner (or any other offline harness)
// can replay a captured plan.
//
// Expected on-disk layout (produced by MppNativeQueryExec.dumpPlanIfEnabled):
...
/// Result of loading a dumped MPP plan. The caller owns these and feeds them
/// to MppQueryCoordinator::create along with a QueryCtx and executor.
struct MppDumpLoadResult {
...
///                       nativeCreateMppQuery does for the runtime-session map
