...
/// producerFragmentId in any FluxExchangeSpec. Its output is consumed by
/// the coordinator via DefaultOutputBufferManager::getData().
struct FluxFragmentSpec {
...
struct FluxExchangeSpec {
  ...
  /// uses (see FluxJniWrapper.cc). Default "ROUND_ROBIN" for backward compat.
...
///
/// Usage:
///   auto coord = FluxQueryCoordinator::create(queryId, fragments, exchanges,
///       queryCtx, executor);
///   coord->start();
...
class FluxQueryCoordinator {
 public:
  /// Factory method.
  static std::shared_ptr<FluxQueryCoordinator> create(
 ...
 private:
  ...
  /// sink; regular FLUX query results continue through the HTTP/Presto path.
  facebook::velox::RowVectorPtr fetchNextDeviceOutput();
#endif

  std::string queryId_;
  std::vector<FluxFragmentSpec> fragmentSpecs_;
  std::vector<FluxExchangeSpec> exchangeSpecs_;
