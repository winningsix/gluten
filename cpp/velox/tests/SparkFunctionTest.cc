...
TEST_F(SparkFunctionTest, netflixToUnixTimeIntegralDefaultFormatUtc) {
  auto dateInts = makeNullableFlatVector<int32_t>({19700101, 20240101, 20230229, std::nullopt});
  auto seconds = evaluate<FlatVector<int64_t>>("nf_to_unixtime(c0, '-', 'Etc/UTC')", makeRowVector({dateInts}));
  auto millis = evaluate<FlatVector<int64_t>>("nf_to_unixtime_ms(c0, '-', 'UTC')", makeRowVector({dateInts}));
...
TEST_F(SparkFunctionTest, netflixDateIntJob175IntegralDefaultFormatUtc) {
  auto values = makeNullableFlatVector<int32_t>(
      {10000101,
       ...
       std::nullopt});
  auto utc = evaluate<FlatVector<int32_t>>("nf_dateint(c0, '-', 'UTC')", makeRowVector({values}));
  auto etcUtc = evaluate<FlatVector<int32_t>>("nf_dateint(c0, '-', 'Etc/UTC')", makeRowVector({values}));
...
TEST_F(SparkFunctionTest, netflixDateIntRejectsFormatAndTimezoneOutsideSlice) {
  ...
  auto explicitFormat = evaluate<FlatVector<int32_t>>("nf_dateint(c0, 'yyyyMMdd', 'UTC')", makeRowVector({values}));
  auto nonUtc = evaluate<FlatVector<int32_t>>("nf_dateint(c0, '-', 'America/Los_Angeles')", makeRowVector({values}));
  auto zulu = evaluate<FlatVector<int32_t>>("nf_dateint(c0, '-', 'Z')", makeRowVector({values}));
...
TEST_F(SparkFunctionTest, netflixFromUnixTimeTzFusedDateIntAndHour) {
  auto epochs = makeNullableFlatVector<int64_t>(
      {1'704'067'200,
       ...
       100'000'000'000'000,
       std::nullopt});
  auto zones = makeNullableFlatVector<StringView>(
      {"UTC", "America/Los_Angeles", "America/Los_Angeles", "America/Los_Angeles", "PST", "+05:30", "UTC", "UTC"});
  auto input = makeRowVector({epochs, zones});
  auto dateInts = evaluate<FlatVector<int32_t>>("nf_dateint_from_unixtime_tz(c0, c1, '-', 'UTC')", input);
  auto hours = evaluate<FlatVector<int32_t>>("nf_hour_from_unixtime_tz(c0, c1, 'Etc/UTC')", input);

  const std::vector<std::optional<int32_t>> expectedDates{
      20240101, 20231231, 20240310, 20240310, 20231231, 20240101, std::nullopt, std::nullopt};
  const std::vector<std::optional<int32_t>> expectedHours{0, 16, 1, 3, 16, 5, std::nullopt, std::nullopt};
  for (size_t row = 0; row < expectedDates.size(); ++row) {
    if (expectedDates[row].has_value()) {
      ASSERT_EQ(dateInts->valueAt(row), *expectedDates[row]);
      ASSERT_EQ(hours->valueAt(row), *expectedHours[row]);
    } else {
      ASSERT_TRUE(dateInts->isNullAt(row));
      ASSERT_TRUE(hours->isNullAt(row));
    }
  }
}

TEST_F(SparkFunctionTest, netflixFromUnixTimeTzFusedRejectsInvalidArguments) {
  auto epochs = makeFlatVector<int64_t>({1'704'067'200});
  auto zones = makeFlatVector<StringView>({"Not/A_Real_Zone"});
  auto input = makeRowVector({epochs, zones});

  auto invalidZone = evaluate<FlatVector<int32_t>>("nf_dateint_from_unixtime_tz(c0, c1, '-', 'UTC')", input);
  auto invalidFormat = evaluate<FlatVector<int32_t>>(
      "nf_dateint_from_unixtime_tz(c0, 'UTC', 'yyyyMMdd', 'UTC')", makeRowVector({epochs}));
  auto invalidOuterZone = evaluate<FlatVector<int32_t>>(
      "nf_hour_from_unixtime_tz(c0, 'UTC', 'America/Los_Angeles')", makeRowVector({epochs}));

  ASSERT_TRUE(invalidZone->isNullAt(0));
  ASSERT_TRUE(invalidFormat->isNullAt(0));
  ASSERT_TRUE(invalidOuterZone->isNullAt(0));
}

TEST_F(SparkFunctionTest, netflixToUnixTimeEpochSecondsAndMillis) {
  auto epochs = makeNullableFlatVector<int64_t>({1704067200, 1704067200000, 100000000000000, std::nullopt});
  auto seconds = evaluate<FlatVector<int64_t>>("nf_to_unixtime(c0, '-', 'UTC')", makeRowVector({epochs}));
  auto millis = evaluate<FlatVector<int64_t>>("nf_to_unixtime_ms(c0, '-', 'Etc/UTC')", makeRowVector({epochs}));
...
TEST_F(SparkFunctionTest, netflixToUnixTimeIntegralThresholds) {
  auto values = makeNullableFlatVector<int64_t>(
      {100000000,
       ...
       std::nullopt});
  auto seconds = evaluate<FlatVector<int64_t>>("nf_to_unixtime(c0, '-', 'UTC')", makeRowVector({values}));
  auto millis = evaluate<FlatVector<int64_t>>("nf_to_unixtime_ms(c0, '-', 'Etc/UTC')", makeRowVector({values}));
