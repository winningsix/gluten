...
// plans directly on Velox-cudf via FluxQueryCoordinator.
//
// Goal: fast iteration loop for debugging TPC-H correctness on the FLUX path.
...
//   2. Compile against the existing FluxQueryCoordinator API as-is.
//   3. Run a hand-constructed two-fragment plan end-to-end.
//   4. Leave clear TODOs for the dump-format parser (blocked on Task #3).
//
// Usage:
//   flux-substrait-runner --dump-dir=<path> [--query=<name>] [--verify=<path>]
//   flux-substrait-runner --builtin-smoke   (run the hand-constructed test)
...
// ---------------------------------------------------------------------------
void printUsage() {
  std::cerr << R"(flux-substrait-runner

Replays Gluten-dumped multi-fragment Substrait plans on Velox-cudf via
FluxQueryCoordinator, bypassing Spark/Gluten/JNI entirely.

Usage:
  flux-substrait-runner --dump-dir=<path> [--query=<name>] [--verify=<path>]
  flux-substrait-runner --builtin-smoke
...
Examples:
  flux-substrait-runner --builtin-smoke
  flux-substrait-runner --dump-dir=/tmp/gluten-dumps --query=q06
)";
}
...
//
// Mirrors the relevant cuDF-related keys from scripts/flux-gpu.template so the
...
std::unordered_map<std::string, std::string> defaultSessionConf() {
  return {
      ...
      // GPU memory tuning. Values copied from scripts/flux-gpu.template so the
...
//   * FluxQueryCoordinator::create/start/next/waitForCompletion.
...
// lives in FluxJniWrapper. Rather than duplicate that plumbing here, we
...
QueryResult runBuiltinSmoke(
  ...
  try {
    ...
    // Wrap into FluxFragmentSpec.
    std::unordered_set<core::PlanNodeId> emptyGroupedIds;
    core::PlanFragment planFragment{
        plan, core::ExecutionStrategy::kUngrouped, 1, emptyGroupedIds};

    gluten::FluxFragmentSpec spec;
    spec.id = 0;
    spec.planFragment = std::move(planFragment);
    spec.numDrivers = 1;

    std::vector<gluten::FluxFragmentSpec> fragments;
    fragments.push_back(std::move(spec));
    std::vector<gluten::FluxExchangeSpec> exchanges; // none: single fragment

    auto coord = gluten::FluxQueryCoordinator::create(
...
//
// Flow mirrors FluxJniWrapper::nativeCreateFluxQuery + nativeStartFluxQuery +
// nativeGetFluxOutput, collapsed into one synchronous pass:
//   1. gluten::loadFluxQueryFromDump() parses the dump and builds specs.
//   2. Build QueryCtx on a dedicated aggregate pool + thread pool sized to
//      the physical driver count.
//   3. FluxQueryCoordinator::create -> start -> drain next() -> wait.
// ---------------------------------------------------------------------------
QueryResult runQueryFromDump(
  ...
  try {
    ...
    auto leafPool = rootPool->addLeafChild("flux-loader-" + r.name);
    auto loaded = gluten::loadFluxQueryFromDump(
    ...
    // Physical driver count: mirror FluxQueryCoordinator's replica-count
    ...
    auto fluxPool = rootPool->addAggregateChild("flux-query-" + r.name);
    std::unordered_map<std::string, std::shared_ptr<config::ConfigBase>>
        connectorConfigs;
    ...
    auto coord = gluten::FluxQueryCoordinator::create(
        queryId,
        std::vector<gluten::FluxFragmentSpec>(loaded.fragments),
        std::vector<gluten::FluxExchangeSpec>(loaded.exchanges),
        queryCtx,
        executor.get());
    coord->start();
...
int main(int argc, char** argv) {
  ...
  gflags::SetUsageMessage(
      "flux-substrait-runner: replay Gluten-dumped multi-fragment Substrait "
      "plans on Velox-cudf via FluxQueryCoordinator.");
  folly::Init init(&argc, &argv, /*removeFlags=*/false);
  google::SetLogDestination(google::GLOG_INFO, "/tmp/flux_runner_info.log");
  ...
  // for FluxQueryCoordinator::next() deserialization. Idempotency-guarded.
  ...
  LOG(WARNING)
      << "flux-substrait-runner: GPU exchange handled by IBM cudf adapters "
      << "(transportType=kUcx); no manual factory registration.";
#else
  LOG(WARNING)
      << "flux-substrait-runner: built without GLUTEN_ENABLE_GPU; "
