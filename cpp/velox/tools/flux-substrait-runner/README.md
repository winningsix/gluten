# Gluten FLUX Dump And Velox-cuDF Replay

This toolchain captures the Spark-Gluten FLUX Substrait fragments that are sent
to native code, then loads the dump directly into Velox-cuDF through
`FluxQueryCoordinator`. It does not require a Presto plan dump.

Use this when you want to debug the Gluten FLUX plan shape or native Velox-cuDF
...
```bash
-Dspark.gluten.mpp.substraitDumpDir=/opt/gluten/flux-dumps-tpch-sf1k
```

Each `FluxNativeQueryExec` writes one subdirectory:
...
The protobuf files are byte-identical to the plan/split bytes that the JNI FLUX
...
export HOST_LOG_ROOT=/home/nfs/ferdinandx/q2/runs/sf1k-q5/CURSOR-flux-dump
export JDK=17
export GPU=0
export PRECOMPILE=false
export SPARK_DRIVER_MAX_RESULT_SIZE=8g
export EXTRA_MVN_ARGS="-Dspark.gluten.mpp.substraitDumpDir=/opt/gluten/flux-dumps-tpch-sf1k"
...
```text
FluxNativeQueryExec: dumped Substrait plan (...) to /opt/gluten/flux-dumps-tpch-sf1k/<query-id>
```
...
```bash
docker cp gluten_q2_ibm:/opt/gluten/flux-dumps-tpch-sf1k \
  /home/nfs/ferdinandx/q2/runs/CURSOR-flux-dumps-tpch-sf1k
```
...
cmake --build cpp/build-dev-e50eb1eaf -j8 --target flux-substrait-runner
```

The binary is written under the build release directory, for example:

```text
cpp/build-dev-e50eb1eaf/releases/flux-substrait-runner
```
...
```bash
/opt/CURSOR-q2-rc-20260506-1-allfix/cpp/build-dev-e50eb1eaf/releases/flux-substrait-runner \
  --dump-dir=/opt/gluten/flux-dumps-tpch-sf1k \
  --query=8 \
  --plan_only \
  --log_level=INFO
```
...
ls -lah /opt/gluten/flux-dumps-tpch-sf1k
ls -lah /opt/gluten/flux-dumps-tpch-sf1k/8
'
```
...
```bash
docker exec gluten_q2_ibm bash -lc '
python3 -m json.tool /opt/gluten/flux-dumps-tpch-sf1k/8/manifest.json | sed -n "1,120p"
python3 -m json.tool /opt/gluten/flux-dumps-tpch-sf1k/8/exchange-specs.json
'
```
...
- If no dump appears, verify `spark.gluten.mpp.enabled=true` and
  ...
  query.
- If `flux-substrait-runner` cannot find a fragment, check that
...
- If plan-only succeeds but replay OOMs, keep `--force_drivers=1`, run one GPU
  job at a time, and inspect `FluxWatchdog` / `FAILED-TASK` logs.
