...
JNIEXPORT void JNICALL Java_org_apache_gluten_init_NativeBackendInitializer_shutdown0( // NOLINT
    JNIEnv* env,
    jclass) {
  JNI_METHOD_START
...
JNIEXPORT jlong JNICALL Java_org_apache_gluten_utils_GpuBufferBatchResizerJniWrapper_create( // NOLINT
    ...
    jobject wrapper,
    jint minOutputBatchSize,
    jlong minOutputBatchSizeInBytes,
    jobject jIter) {
  JNI_METHOD_START
  auto ctx = getRuntime(env, wrapper);
  ...
  auto appender = std::make_shared<ResultIterator>(std::make_unique<GpuBufferBatchResizer>(
      arrowPool, pool.get(), minOutputBatchSize, minOutputBatchSizeInBytes, std::move(iter)));
  return ctx->saveObject(appender);
  JNI_METHOD_END(kInvalidObjectHandle)
}
#endif
