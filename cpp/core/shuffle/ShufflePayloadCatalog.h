...
class ShufflePayloadCatalog {
 public:
  ...
  struct ResolveResult {
    ...
    std::vector<std::pair<const uint8_t*, int64_t>>
        segments;
  };

  // Combined has+resolve in one call with shared_lock.
  ResolveResult resolveBlockDirect(
      int32_t shuffleId,
      int64_t mapId,
      int32_t partitionId);
 ...
 private:
  ...
  struct BlockKeyHash {
    size_t operator()(const BlockKey& k) const {
      ...
      h ^= std::hash<int64_t>()(k.mapId) + 0x9e3779b9 +
          (h << 6) + (h >> 2);
      h ^= std::hash<int32_t>()(k.partitionId) +
          0x9e3779b9 + (h << 6) + (h >> 2);
      return h;
    }
  };
  ...
  std::unordered_map<BlockKey, PartitionEntry, BlockKeyHash>
      catalog_;
  ...
  std::unordered_map<int32_t, std::vector<MapKey>>
      shuffleIndex_;
};
...
class MultiSegmentInputStream
 ...
 public:
  explicit MultiSegmentInputStream(
      std::vector<std::shared_ptr<arrow::Buffer>> segments);

  arrow::Status Close() override;
  bool closed() const override;
  arrow::Result<int64_t> Tell() const override;
  arrow::Result<int64_t> Read(
      int64_t nbytes, void* out) override;
